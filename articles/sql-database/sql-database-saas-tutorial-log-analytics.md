...
---
# <a name="setup-and-use-log-analytics-oms-with-the-wingtip-saas-app"></a>Konfiguracja i analiza dzienników (OMS) za pomocą aplikacji Wingtip SaaS

W tym samouczku, konfigurowania i używania *analizy dzienników ([OMS](https://www.microsoft.com/cloud-platform/operations-management-suite))* monitorowania pule elastyczne i baz danych. Ten samouczek nawiązuje do [samouczka zarządzania i monitorowania wydajności](sql-database-saas-tutorial-performance-monitoring.md) i pokazuje, jak używać aplikacji *Log Analytics* do rozszerzania, monitorowania i zgłaszania alertów w witrynie Azure Portal. Program Log Analytics jest szczególnie przydatny w przypadku monitorowania i alertów na dużą skalę, ponieważ obsługuje setki pul i setki tysięcy baz danych. Udostępnia również jedno rozwiązanie do monitorowania, w którym można zintegrować monitorowanie różnych aplikacji i usług Azure w wielu subskrypcjach Azure.
