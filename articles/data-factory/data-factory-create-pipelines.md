...
Ten artykuł pomaga zrozumieć potoki i działań w fabryce danych Azure i używać ich do tworzenia przepływów pracy opartych na danych end-to-end przenoszenia danych i scenariusze przetwarzania danych.  
...
Fabryka danych może obejmować jeden lub wiele potoków. Potok to logiczne grupowanie działań wykonujących zadania. Działania w potoku, zdefiniuj akcje do wykonania na podstawie danych. Możesz na przykład użyć działania kopiowania w celu skopiowania danych z lokalnego programu SQL Server do usługi Azure Blob Storage. Następnie użyj działania usługi Hive, które uruchamia skrypt Hive w klastrze usługi Apache HDInsight w celu przetworzenia/przekształcenia danych z magazynu obiektów blob, aby utworzyć dane wyjściowe. Na koniec użyj drugi działanie kopiowania do kopiowania danych wyjściowych, aby Magazyn danych SQL Azure, na które raportowania rozwiązań analizy biznesowej (BI) są tworzone. 

Działanie może zająć zero lub więcej danych wejściowych [zestawów danych](data-factory-create-datasets.md) i tworzące co najmniej jeden wyjściowy [zestawów danych](data-factory-create-datasets.md). Na poniższym diagramie przedstawiono relację między potoku, działania i zestawu danych w fabryce danych: 
...
Zestaw danych wejściowych reprezentuje dane wejściowe dla działania w potoku a wyjściowy zestaw danych wyjściowych dla działania. Zestawy danych identyfikowanie danych między magazynami różne dane, takie jak tabele, plików, folderów i dokumentów. Po utworzeniu zestawu danych, można go z działaniami w potoku. Na przykład zestawu danych może być zestawem danych wejścia/wyjścia aktywności kopiowania lub HDInsightHive działania. Aby uzyskać więcej informacji na temat zestawów danych, zobacz [zestawów danych w fabryce danych Azure](data-factory-create-datasets.md) artykułu.
...
## <a name="pipeline-json"></a>Kod JSON potoku
Daj nam zająć bliższe spojrzenie na jak potoku jest zdefiniowany w formacie JSON. Struktura ogólna dla potoku wygląda następująco:
...
| Tag | Opis | Wymagane |
| --- | --- | --- |
| name |Nazwa potoku. Określ nazwę, która reprezentuje akcję, która wykonuje potoku. <br/><ul><li>Maksymalna liczba znaków: 260</li><li>Musi rozpoczynać się od numeru litery lub znaku podkreślenia (_)</li><li>Następujące znaki nie są dozwolone: ".", "+","?", "/", "<",">", "*", "%", "&", ":","\\"</li></ul> |Tak |
| Opis elementu | Określ opis przeznaczenia potoku. |Tak |
| działania | **Działania** sekcji może mieć co najmniej jednego działania zdefiniowane w nim. Zobacz następną sekcję, aby uzyskać szczegółowe informacje dotyczące działań elementu JSON. | Tak |  
...
## <a name="activity-json"></a>Aktywność JSON
**Działania** sekcji może mieć co najmniej jednego działania zdefiniowane w nim. Każde działanie ma następującą strukturę najwyższego poziomu:
...
Poniższa tabela zawiera opis właściwości w działaniu definicji JSON:

| Tag | Opis | Wymagane |
| --- | --- | --- |
| name | Nazwa działania. Określ nazwę, która reprezentuje akcję wykonywaną przez działanie. <br/><ul><li>Maksymalna liczba znaków: 260</li><li>Musi rozpoczynać się od numeru litery lub znaku podkreślenia (_)</li><li>Następujące znaki nie są dozwolone: ".", "+","?", "/", "<",">", "*", "%", "&", ":","\\"</li></ul> |Tak |
| Opis elementu | Tekst opisujący jakiego działania lub służy do |Tak |
| type | Typ działania. Zobacz [działań przepływu danych](#data-movement-activities) i [działań przekształcania danych](#data-transformation-activities) sekcje dla różnych typów działań. |Tak |
...
| linkedServiceName |Nazwa połączonej usługi używana przez działanie. <br/><br/>Działanie może wymagać określenia połączonej usługi, który stanowi łącze do środowiska obliczeniowego wymagane. |Tak w przypadku działania usługi HDInsight i Azure Machine Learning działanie wsadowego oceniania <br/><br/>Nie dla wszystkich innych |
| typeProperties |Właściwości w **typeProperties** sekcji są zależne od typu działania. Aby wyświetlić właściwości typu dla działania, kliknij łącza do działania w poprzedniej sekcji. | Nie |
...
* Dane wejściowe dla działania mają ustawienie **InputDataset**, a dane wyjściowe — **OutputDataset**. Zobacz [zestawów danych](data-factory-create-datasets.md) artykułu do definiowania zestawów danych w formacie JSON. 
...
**TypeProperties** sekcja jest różne dla każdego działania transformacji. Informacje na temat właściwości typu obsługiwane dla działania przekształcania, kliknij działanie transformacji w [działań przekształcania danych](#data-transformation-activities) tabeli. 
...
Poprzednie potoki dwa przykładowe zawierać tylko jedno działanie w nich. Może zawierać więcej niż jedno działanie w potoku.  
...
- [Tworzenie potoku z działaniem przekształcenia danych](data-factory-build-your-first-pipeline.md)
