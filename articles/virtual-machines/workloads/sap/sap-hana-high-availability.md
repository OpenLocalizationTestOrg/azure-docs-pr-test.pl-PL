...
W infrastrukturze lokalnej, możesz użyć albo HANA replikacji systemu lub magazynu udostępnionego tooestablish wysokiej dostępności dla SAP HANA.
Aktualnie obsługiwany jest tylko skonfigurowanie HANA replikacji systemu Azure. SAP HANA replikacji składa się z jednego węzła głównego i co najmniej jeden podrzędny węzła. Toohello zmiany, danych w węźle głównym hello są replikowane węzłów podrzędnych toohello synchronicznie lub asynchronicznie.

W tym artykule opisano sposób toodeploy hello maszyn wirtualnych, konfigurowanie maszyn wirtualnych hello, zainstaluj program hello framework klastra, zainstalować i skonfigurować replikacji systemu SAP HANA.
W hello przykładowe konfiguracje instalacja poleceń itp. liczby wystąpień 03 i HANA systemu identyfikator HDB jest używany.
...
* [SAP HANA SR wydajności zoptymalizowanych pod kątem scenariusza] [ suse-hana-ha-guide] hello przewodnik zawiera wszystkie wymagane informacje tooset zapasowych replikacji systemu SAP HANA lokalnie. Użyj tego podręcznika, jako linii bazowej.

## <a name="deploying-linux"></a>Wdrażanie systemu Linux

Hello agenta zasobów dla SAP HANA znajduje się w systemie SUSE Linux Enterprise Server aplikacje SAP.
Hello Azure Marketplace zawiera obraz systemu SUSE Linux Enterprise Server dla programu SAP 12 aplikacji z użyciem nowych maszyn wirtualnych toodeploy BYOS (Bring Your własnej subskrypcji).
...
1. Konfigurowanie równoważenia obciążenia hello
    1. Utwórz pulę IP frontonu
        1. Otwórz moduł równoważenia obciążenia hello, wybierz puli adresów IP frontonu i kliknij przycisk Dodaj
        1. Wprowadź nazwę hello puli hello do adresów IP nowego serwera sieci Web (na przykład frontonu hana)
       1. Kliknij przycisk OK
        1. Po utworzeniu hello nowa pula IP frontonu, zanotuj jego adresu IP
    1. Tworzenie puli wewnętrznej bazy danych
        1. Otwórz moduł równoważenia obciążenia hello, wybierz pul zaplecza i kliknij przycisk Dodaj
        1. Wprowadź nazwę hello hello nowej puli wewnętrznej bazy danych (na przykład zaplecze hana)
        1. Kliknij przycisk Dodaj maszynę wirtualną
        1. Wybierz hello utworzoną wcześniej zestawu dostępności
        1. Wybierz maszyny wirtualne hello hello SAP HANA klastra
        1. Kliknij przycisk OK
    1. Utworzyć sondy kondycji
       1. Otwórz moduł równoważenia obciążenia hello, wybierz sond kondycji i kliknij przycisk Dodaj
        1. Wprowadź nazwę hello hello nowe sondy kondycji (na przykład hp hana)
        1. Wybierz protokół, port 625 TCP**03**, interwał 5 i próg złej kondycji 2
        1. Kliknij przycisk OK
    1. Tworzenie reguł równoważenia obciążenia
        1. Otwórz moduł równoważenia obciążenia hello, wybierz reguły równoważenia obciążenia i kliknij przycisk Dodaj
        1. Wprowadź nazwę hello hello nowej reguły modułu równoważenia obciążenia (na przykład hana-lb-3**03**15)
        1. Adres IP frontonu wybierz hello, puli wewnętrznej bazy danych i kondycji sondowania utworzonego wcześniej (na przykład frontonu hana)
        1. Zachowaj protokołu TCP, wprowadź port 3**03**15
        1. Zwiększ limit czasu bezczynności too30 minut
       1. **Upewnij się, że tooenable pływającego adresu IP**
        1. Kliknij przycisk OK
        1. Powtórz powyższe kroki hello portu 3**03**17
...
1. Otwórz hello [szablonu bazy danych] [ template-multisid-db] lub hello [zbieżność szablonu] [ template-converged] w portalu Azure hello hello szablonu bazy danych tylko tworzy hello reguły równoważenia obciążenia dla bazy danych należy szablon konwergentnej hello również tworzy tekst hello reguły równoważenia obciążenia ASCS/SCS i wystąpienia Wywołujących (tylko w systemie Linux). Jeśli planujesz tooinstall systemu SAP NetWeaver na podstawie i ma tooinstall hello ASCS/SCS wystąpienia na powitania sam maszyny, użyj hello [zbieżność szablonu][template-converged].
1. Wprowadź następujące parametry hello
    1. Identyfikator systemu SAP  
       Wprowadź systemu SAP hello identyfikator hello ma tooinstall systemu SAP. Witaj identyfikator będzie służyć jako prefiks hello zasobów, które zostały wdrożone.
    1. Typ stosu (dotyczy tylko jeśli używasz szablonu konwergentnej hello)  
    ...
    1. Rozmiar systemu SAP  
       zapewni Hello ilość protokoły SAP hello nowy system. Jeśli nie masz pewności, ile protokoły SAP hello system będzie wymagać, należy skontaktować się z partnerem technologii SAP lub Integrator systemu
    ...
    1. Nowy lub istniejący podsieci  
       Określa, czy należy utworzyć nową sieć wirtualną i podsieć lub istniejącą podsieć powinna być używana. Jeśli masz już sieć wirtualną tooyour połączonych sieci lokalnej, wybierz istniejącą.
    1. Identyfikator podsieci  
    Identyfikator Hello maszyn wirtualnych hello hello podsieci toowhich powinny być podłączone do. Wybierz podsieć hello sieci VPN lub usługi Express Route sieci wirtualnej tooconnect hello maszyny wirtualnej tooyour w sieci lokalnej. Identyfikator Hello zwykle wygląda /subscriptions/`<subscription id`> /resourceGroups/`<resource group name`> /providers/Microsoft.Network/virtualNetworks/`<virtual network name`> /subnets/`<subnet name`>

## <a name="setting-up-linux-ha"></a>Konfigurowanie HA systemu Linux

Witaj, następujące elementy są poprzedzane prefiksem albo [A] - tooall odpowiednich węzłów, toonode przeznaczone wyłącznie [1] - toonode stosuje się tylko 1 lub [2] - 2.

1. [A] SLES dla SAP BYOS tylko - zarejestrować SLES toobe toouse stanie hello repozytoria
...
1. [A] układ dysku instalacji
    1. LVM  
    Ogólnie zaleca toouse LVM dla woluminów, które przechowują dane i pliki dziennika. w poniższym przykładzie Hello założenia, że maszyny wirtualne hello czterech dyskach danych dołączonych, które powinny być używane toocreate dwa woluminy.
    ...
    sudo blkid
        ...
        * Utwórz wpisy fstab hello trzy logiczne woluminy
    <pre><code>
    sudo vi /etc/fstab
    </code></pre>
    Wstaw ten wiersz zbyt/etc/fstab
    <pre><code>
    ...
    # <a name="write-down-hello-id-of-devsdc1"></a>Zanotuj identyfikator hello /dev/sdc1
...
1. [A] zmiany hacluster hasło toohello tego samego hasła
    ```bash
    sudo passwd hacluster
    
    ```

1. [A] skonfiguruj corosync toouse innych transportu i dodać do wstawienia. Klaster nie będą działać inaczej.
    ```bash
...
Wykonaj rozdział 4 hello [przewodnik SAP HANA SR wydajności zoptymalizowanych pod kątem scenariusza] [ suse-hana-ha-guide] tooinstall replikacji systemu SAP HANA.

1. [A] uruchamiania hdblcm z hello HANA DVD
    ...
    * Czy chcesz, aby system toohello dodatkowych hostów tooadd? (t/n) [[n]: -> ENTER
    * Wprowadź identyfikator systemu SAP HANA:<SID of HANA e.g. HDB>
    * Wprowadź liczby wystąpień [00]:   
  Liczby wystąpień HANA. Użyj 03, jeśli używany szablon Azure hello i stosować w powyższym przykładzie hello
    * Wybierz tryb bazy danych / wprowadź indeks [1]: -> ENTER
    * Użycie systemu wybierz / Wprowadź indeks [4]:  
  Wybierz system hello użycia
    ...
    * Czy chcesz toocontinue? (t/n):  
  Sprawdź poprawność hello podsumowania, a następnie wprowadź y toocontinue
...
1. [1] Przełącz toohello sapsid użytkownika (na przykład hdbadm) i utworzyć hello lokacji głównej.
    <pre><code>
    su - <b>hdb</b>adm
    hdbnsutil -sr_enable –-name=<b>SITE1</b>
    </code></pre>
1. [2] Przełącz toohello sapsid użytkownika (na przykład hdbadm) i utworzyć lokację dodatkową hello.
    <pre><code>
    su - <b>hdb</b>adm
...
Zmień ustawienia domyślne hello
...
# <a name="now-we-load-hello-file-toohello-cluster"></a>Teraz możemy załadować hello pliku toohello klastra
...
powitania po rozdziale opisano, jak można przetestować konfigurację. Każdy test założono, że są głównymi i wzorzec SAP HANA hello jest uruchomiona na powitania saphanavm1 maszyny wirtualnej.

#### <a name="fencing-test"></a>Test ogrodzenia

Instalator hello hello ogrodzenia agenta można przetestować wyłączając hello interfejs sieciowy na saphanavm1 węzła.

<pre><code>
sudo ifdown eth0
</code></pre>

teraz Pobierz ponownie uruchomiona lub zatrzymana w zależności od konfiguracji klastra Hello maszyny wirtualnej.
Jeśli ustawisz toooff akcji stonith hello hello maszyny wirtualnej zostanie zatrzymana i zasoby hello są migrowane toohello uruchomienie maszyny wirtualnej.

Po ponownym uruchomieniu maszyny wirtualnej hello hello zasobów SAP HANA zakończy się niepowodzeniem toostart pomocniczym po ustawieniu AUTOMATED_REGISTER = "false". W takim przypadku należy tooconfigure hello HANA wystąpienia jako dodatkowej, wykonując następujące polecenia hello:
...
Zatrzymywanie usługi rozrusznik hello na węzeł saphanavm1 możesz przetestować ręcznego przełączania trybu failover.
<pre><code>
service pacemaker stop
</code></pre>

Po hello w tryb failover można ponownie uruchomić usługę hello. Witaj SAP HANA zasobu na saphanavm1 zakończy się niepowodzeniem toostart pomocniczym po ustawieniu AUTOMATED_REGISTER = "false". W takim przypadku należy tooconfigure hello HANA wystąpienia jako dodatkowej, wykonując następujące polecenia hello:
...
Węzeł główny SAP HANA hello można migrować, wykonując następujące polecenia hello
<pre><code>
crm resource migrate msl_SAPHana_<b>HDB</b>_HDB<b>03</b> <b>saphanavm2</b>
crm resource migrate g_ip_<b>HDB</b>_HDB<b>03</b> <b>saphanavm2</b>
</code></pre>

To należy zmigrować węzła głównego SAP HANA hello i hello grupy, która zawiera hello toosaphanavm2 w wirtualnego adresu IP.
Witaj SAP HANA zasobu na saphanavm1 zakończy się niepowodzeniem toostart pomocniczym po ustawieniu AUTOMATED_REGISTER = "false". W takim przypadku należy tooconfigure hello HANA wystąpienia jako dodatkowej, wykonując następujące polecenia hello:
...
Migracja Hello tworzy ograniczenia lokalizacji, w których należy ponownie usunąć toobe.
...
Należy również toocleanup hello stanu zasobu węzła pomocniczego hello
