...
---
# <a name="how-toouse-packer-toocreate-windows-virtual-machine-images-in-azure"></a>Jak obrazy maszyny wirtualnej systemu Windows toocreate pakujący toouse na platformie Azure
Każda maszyna wirtualna (VM) na platformie Azure jest tworzony z obrazu, który definiuje hello dystrybucji systemu Windows i wersji systemu operacyjnego. Obrazy mogą obejmować wstępnie zainstalowane aplikacje i konfiguracje. Hello Azure Marketplace zawiera wiele obrazów pierwszy i innych firm dla najbardziej typowych systemu operacyjnego i środowisk aplikacji albo można utworzyć własne niestandardowe obrazy dostosowane tooyour potrzeb. W tym artykule szczegółowo sposób toouse hello otworzyć narzędzie źródła [pakujący](https://www.packer.io/) toodefine i kompilacji niestandardowych obrazów na platformie Azure.
...
Utwórz nową grupę zasobów o [New-AzureRmResourceGroup](/powershell/module/azurerm.resources/new-azurermresourcegroup). Witaj poniższy przykład tworzy grupę zasobów o nazwie *myResourceGroup* w hello *eastus* lokalizacji:
...
tooauthenticate tooAzure, należy również tooobtain Azure identyfikatorów dzierżawcy i subskrypcji z [Get-AzureRmSubscription](/powershell/module/azurerm.profile/get-azurermsubscription):
...
Za pomocą tych dwóch identyfikatorów w hello następnego kroku.
...
Utwórz plik o nazwie *windows.json* i Wklej powitania po zawartości. Wprowadź własne wartości dla następujących hello:
...
Ten szablon tworzy Maszynę wirtualną systemu Windows Server 2016, instaluje usługi IIS, a następnie stanowi uogólnienie hello maszyny Wirtualnej przy użyciu narzędzia Sysprep.
...
Go zajmuje kilka minut hello toobuild pakujący maszynę Wirtualną, uruchom hello provisioners i czyszczenie hello wdrożenia.
...
Można teraz utworzyć Maszynę wirtualną z obrazu z [AzureRmVM nowy](/powershell/module/azurerm.compute/new-azurermvm). Witaj poniższy przykład tworzy Maszynę wirtualną o nazwie *myVM* z *myPackerImage*.
...
Może potrwać kilka minut hello toocreate maszyny Wirtualnej.


## <a name="test-vm-and-iis"></a>Test maszyny Wirtualnej i IIS
Uzyskaj hello publicznego adresu IP maszyny Wirtualnej z [Get-AzureRmPublicIPAddress](/powershell/module/azurerm.network/get-azurermpublicipaddress). Witaj poniższy przykład uzyskuje hello adres IP dla *myPublicIP* utworzony wcześniej:
...
W przeglądarce sieci web tooa można następnie wprowadzić hello publicznego adresu IP.

![Domyślna witryna usług IIS](./media/build-image-with-packer/iis.png) 


## <a name="next-steps"></a>Następne kroki
W tym przykładzie użyto toocreate pakujący obrazu maszyny Wirtualnej z już zainstalowanymi usługami IIS. Można użyć tego obrazu maszyny Wirtualnej będą widoczne obok istniejących przepływów pracy wdrażania, takie jak toodeploy Twojego tooVMs aplikacji utworzone na podstawie hello obrazu za pomocą usługi Team Services, Ansible, Chef lub Puppet.
