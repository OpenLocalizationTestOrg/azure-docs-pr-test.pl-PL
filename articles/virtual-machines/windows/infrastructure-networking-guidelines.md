...
Ponieważ nie podłączaj tooyour sieci lokalnej, sieci wirtualnych wyłącznie do usługi Azure można używać jakiejkolwiek jego części hello prywatnych przestrzeni adresów IP, nawet jeśli hello jest tą samą przestrzenią prywatnej użyć lokalnego.

### <a name="cross-premises-virtual-networks"></a>Między różnymi lokalizacjami, sieci wirtualnych
Jeśli lokalnych użytkowników i komputery wymagają tooVMs trwającą łączność w sieci wirtualnej platformy Azure, należy utworzyć sieć wirtualną między lokalizacjami.  Podłącz je tooyour sieci lokalnej przy użyciu usługi ExpressRoute lub połączeń sieci VPN lokacja lokacja.
...
Podobnie jak w przypadku fizycznej infrastruktury sieci lokalnej, sieci wirtualne platformy Azure może zawierać więcej niż podsieci i adresów IP. Podczas projektowania infrastruktury aplikacji, może być tooincorporate niektóre z tych dodatkowych składników:
