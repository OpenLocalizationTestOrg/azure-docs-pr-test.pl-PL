...
terraform init
```
 Dostawca Terraform hello tooinitialize dla platformy Azure. Zmień katalog hello zbyt**terraformscripts**, i hello problem następujące polecenie:
```
terraform plan
```
...
* Konto magazynu dla hello diagnostyki maszyny wirtualnej
* Publiczny adres IP
...
~~~~
W tym miejscu utworzono konto magazynu. To konto magazynu jest, gdzie hello maszyny wirtualnej będzie przechowywać jego szczegóły diagnostyki.
...
~~~~
Na koniec fragment poprzedniej hello tworzy maszyny wirtualnej, która wykorzystuje wszystkie zasoby hello już udostępniane. Są one konto magazynu diagnostyki maszyny wirtualnej hello, karty sieciowej z publicznych adresów IP i określonej podsieci i grupy zasobów hello utworzone. Należy pamiętać, właściwość vm_size hello, gdzie hello skryptu Określa standardowy SKU DS1v2 Azure.

Jesteś toosupply wymagany klucz publiczny SSH. Umieść hello wartość klucza publicznego w sekcji hello **... WSTAW PUBLICZNY KLUCZ OPENSSH TUTAJ...**  powyżej. Można użyć istniejącego ssh parę kluczy lub wykonaj hello [Windows](https://docs.microsoft.com/en-us/azure/virtual-machines/linux/ssh-from-windows) lub [Linux/macOS](https://docs.microsoft.com/en-us/azure/virtual-machines/linux/mac-create-ssh-keys) pary kluczy hello toogenerate dokumentacji.
