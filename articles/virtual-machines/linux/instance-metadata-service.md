...
---
# <a name="azure-instance-metadata-service-for-linux-vms"></a>Usługa Azure wystąpienie metadanych dla maszyn wirtualnych systemu Linux
...
> [!IMPORTANT]
> Ta usługa jest **ogólnie dostępna** w globalnej regiony platformy Azure. Znajduje się w publicznej wersji zapoznawczej dla instytucji rządowych, Chin i chmury Azure niemiecki. Regularnie odbiera aktualizacje, aby udostępnić nowe informacje o wystąpieniu maszyny wirtualnej. Ta strona odzwierciedla aktualne [kategorii danych](#instance-metadata-data-categories) dostępne.

## <a name="service-availability"></a>Dostępność usług
Usługa jest dostępna we wszystkich regionach globalne Azure ogólnie dostępna. Usługa jest w wersji zapoznawczej w regionach dla instytucji rządowych, Chiny lub Niemczech.
...
Ta tabela jest aktualizowany, gdy usługa stanie się dostępny w innych chmur platformy Azure.
...
Wystąpienie usługi metadanych jest kontrolą wersji. Wersje są obowiązkowe i bieżąca wersja to `2017-04-02`.
...
Przy wdrażaniu nowsze wersje, starsze wersje nadal będą dostępne dla zgodności czy skrypty są zależne formatów określonych danych. Jednak należy pamiętać, że bieżący version(2017-03-01) Podgląd mogą nie być dostępne po usługi jest ogólnie dostępna.
...
Domyślnie usługa metadanych wystąpienia zwraca dane w formacie JSON (`Content-Type: application/json`). Jednak różnych interfejsów API może zwracać dane w różnych formatach, jeśli jest to wymagane.
...
404 — Nie odnaleziono | Istnieje does't żądanego elementu 
...
Dane | Opis
-----|------------
location | Region platformy Azure, maszyna wirtualna jest uruchomiona
name | Nazwa maszyny Wirtualnej 
Oferta | Oferują informacji o obrazie maszyny Wirtualnej. Ta wartość ma tylko obrazy wdrożone z galerii Azure obrazu.
Wydawcy | Wydawcy obrazu maszyny Wirtualnej
Jednostka SKU | Określonej jednostki SKU dla obrazu maszyny Wirtualnej  
Wersja | Wersja obrazu maszyny Wirtualnej 
osType | Linux lub Windows 
platformUpdateDomain |  [Domeny aktualizacji](manage-availability.md) wirtualna jest uruchomiona
platformFaultDomain | [Domena błędów](manage-availability.md) wirtualna jest uruchomiona
...
IPv4/elementu privateIpAddress | Lokalny adres IPv4 maszyny wirtualnej 
IPv4/publicznego adresu IP | Publiczny adres IPv4 maszyny wirtualnej
podsieć lub adres. | Adres podsieci maszyny wirtualnej
prefiks podsieci / | Prefiks podsieci, przykład 24
adres IPv6/IP | Lokalny adres IPv6 maszyny wirtualnej
...
4. Dlaczego otrzymuję błąd `500 Internal Server Error`?
...
6. Jak uzyskać pomoc techniczną dla usługi?
