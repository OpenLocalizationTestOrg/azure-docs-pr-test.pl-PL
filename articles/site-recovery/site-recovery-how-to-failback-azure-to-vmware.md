...
* Może się nie powieść kopii tylko do plików maszyny wirtualnej systemu (VMFS) Magazyn danych. Jeśli masz vSAN lub RDM ponownej ochrony i powrotu po awarii nie będzie działać.
...
* Do dysków opartych na sieci vSAN lub RDM nie może zakończyć się niepowodzeniem. Na VMFS magazyn danych można tworzyć tylko nowe dyski maszyny wirtualnej (VMDKs).
...
* Serwer konfiguracji jest wymagana lokalnie czy powrotu po awarii. Podczas powrotu po awarii maszyna wirtualna musi istnieć w bazie danych konfiguracji serwera lub powrotu po awarii nie powiodło się. W związku z tym upewnij się, wykonanie zaplanowanego tworzenia kopii zapasowych serwera. Jeśli wystąpił awarii, należy przywrócić działanie serwera przy użyciu tego samego adresu IP do powrotu po awarii do pracy.
* Główny serwer docelowy nie powinna mieć żadnych migawek przed wyzwalania powrotu po awarii.
