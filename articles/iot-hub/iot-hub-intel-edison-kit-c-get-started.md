...
---
# <a name="connect-intel-edison-tooazure-iot-hub-c"></a>Połącz tooAzure Intel Edison IoT Hub (C)
...
1. Użyj jednej z poniższych klientów SSH z tooyour tooconnect Twojego hosta komputera Intel Edison hello. adres IP Hello jest z narzędzia do konfiguracji hello oraz hasło hello jest hello jedną ustawione w tym narzędziu.
    ...
    - Witaj wbudowanego klienta SSH Ubuntu lub macOS (Uruchom `ssh root@"hello IP address"`).
...
3. Następnie przejdź toohello repozytorium folder toorun hello następujące polecenia toobuild Azure IoT SDK
...
1. Otwórz plik konfiguracji hello, uruchamiając następujące polecenia hello:
   ...
   Istnieją dwa makra w tym pliku mogą configurate. Witaj najpierw jest `INTERVAL`, który definiuje dwa komunikaty, które wysyłają toocloud hello interwału. Witaj drugi `SIMULATED_DATA`, czyli wartość logiczną parametru czy toouse symulowane dane czujników, czy nie.
