...
1. Otwórz plik konfiguracji, uruchamiając następujące polecenia:
   ...
   Istnieją dwa elementy w tym pliku mogą configurate. Pierwsza z nich jest `interval`, która określa przedział czasu (w milisekundach) między dwa komunikaty, które wysyłają do chmury. Drugi `simulatedData`, która jest wartość logiczna, czy należy użyć danych czujnika symulowane lub nie.
