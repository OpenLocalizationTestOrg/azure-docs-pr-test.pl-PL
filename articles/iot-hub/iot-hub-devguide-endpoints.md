...
* **Zarządzanie tożsamościami urządzenia**. Każdy Centrum IoT udostępnia zestaw punktów końcowych REST protokołu HTTP do zarządzania tożsamościami urządzenia (Tworzenie, pobieranie, aktualizowanie i usuwanie). [Tożsamości urządzenia] [ lnk-device-identities] służą do urządzenia uwierzytelniania i kontroli dostępu.
* **Zarządzanie urządzeniami dwie**. Każdy Centrum IoT udostępnia zestaw punkt końcowy REST protokołu HTTP skierowane do usługi do zapytania i aktualizacji [twins urządzenia] [ lnk-twins] (aktualizacja znaczniki i właściwości).
* **Zadania zarządzania**. Każdy Centrum IoT udostępnia zestaw połączonej usługi REST protokołu HTTP z punktu końcowego do wykonywania zapytań i zarządzanie [zadania][lnk-jobs].
