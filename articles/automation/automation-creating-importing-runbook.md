...
4. Nowy element runbook będą wyświetlane na **elementów Runbook** kartę.
...
Nowy element runbook automatyzacji Azure można utworzyć, importując skrypt programu PowerShell lub przepływu pracy programu PowerShell (z rozszerzeniem .ps1) lub wyeksportowany graficzny element runbook (graphrunbook).  Należy określić [typu element runbook](automation-runbook-types.md) który zostanie utworzony na podstawie importu, biorąc pod uwagę przedstawione poniżej zagadnienia.
...
5. Nowy element runbook będzie wyświetlany na **elementów Runbook** karty dla konta automatyzacji.
...
6. [Typ elementu runbook](automation-runbook-types.md) zostanie automatycznie wybrana, ale można zmienić typ po przejęciu zastosowanie ograniczeń pod uwagę. 
7. Nowy element runbook będą wyświetlane na liście elementów runbook dla konta automatyzacji.
8. Należy [opublikować elementu runbook](#publishing-a-runbook) przed jej uruchomieniem.

> [!NOTE]
> Po zaimportowaniu graficzny element runbook lub graficznym elementem runbook przepływu pracy programu PowerShell, istnieje możliwość można przekonwertować na inny typ, jeśli chce. Nie można przekonwertować tekstową.
> 
> 
