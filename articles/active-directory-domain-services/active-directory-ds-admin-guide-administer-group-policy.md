...
1. Przejdź do **maszyn wirtualnych** węzła w klasycznym portalu Azure. Wybierz maszyny wirtualne utworzone w zadaniu 1, a następnie kliknij przycisk **Connect** na pasku poleceń u dołu okna.

    ![Połącz z maszyną wirtualną systemu Windows](./media/active-directory-domain-services-admin-guide/connect-windows-vm.png)
2. Klasyczny portal wyświetli monit o otworzyć lub zapisać plik z rozszerzeniem "RDP", który jest używany do nawiązania połączenia z maszyną wirtualną. Kliknij plik, po zakończeniu pobierania.
3. W wierszu logowania Użyj poświadczeń użytkownika należącego do grupy "Administratorzy usługi AAD kontrolera domeny". Na przykład używamy "bob@domainservicespreview.onmicrosoft.com" w tym przypadku.
