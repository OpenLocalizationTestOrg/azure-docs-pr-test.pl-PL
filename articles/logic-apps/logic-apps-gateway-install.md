...
Brama działa jako usługa systemu Windows. Podobnie jak inne usługi systemu Windows można uruchomić i zatrzymać usługę na wiele sposobów. Można na przykład, otwórz wiersz polecenia z podwyższonym poziomem uprawnień na komputerze, na którym jest uruchomiona brama i uruchamiania obu tych poleceń:
...
Brama danych lokalnych jest skonfigurowany do użycia `NT SERVICE\PBIEgwService` dla systemu Windows usługi poświadczeń logowania. Domyślnie bramy ma prawo "Zaloguj jako usługa" dla komputera, w którym zainstalowano bramę.
