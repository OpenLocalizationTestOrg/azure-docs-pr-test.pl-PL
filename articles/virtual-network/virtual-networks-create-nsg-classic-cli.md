...
---
# <a name="how-toocreate-nsgs-classic-in-hello-azure-cli"></a>Jak grupy NSG toocreate (klasyczne) w hello wiersza polecenia platformy Azure
...
W tym artykule omówiono hello klasycznego modelu wdrażania. Możesz również [tworzenia grup NSG w modelu wdrażania usługi Resource Manager hello](virtual-networks-create-nsg-arm-cli.md).

[!INCLUDE [virtual-networks-create-nsg-scenario-include](../../includes/virtual-networks-create-nsg-scenario-include.md)]

Poniższe polecenia interfejsu wiersza polecenia Azure próbki Hello oczekiwać środowisku niezłożonym już utworzone w zależności od scenariusza hello powyżej. Jeśli chcesz korzystać z poleceń hello toorun wyświetlaną w tym dokumencie, najpierw utworzyć środowisko testowe hello przez [tworzenia sieci wirtualnej](virtual-networks-create-vnet-classic-cli.md).
...
2. Uruchom hello  **`azure config mode`**  polecenia tooswitch tooclassic tryb, jak pokazano poniżej.
...
3. Uruchom hello  **`azure network nsg create`**  toocreate polecenia grupy NSG.
...
4. Uruchom hello  **`azure network nsg rule create`**  toocreate polecenia regułę, która umożliwia dostęp tooport 3389 (RDP) z hello Internet.
   ...
   * **-c (lub--akcji)**. Poziom dostępu dla reguły hello (Deny lub Zezwalaj).
...
5. Uruchom hello  **`azure network nsg rule create`**  toocreate polecenia regułę, która umożliwia dostęp tooport 80 (HTTP) z hello Internet.
    ...
    Oczekiwano putput:
        ...
        data:    Priority                        : 200
        info:    network nsg rule create command OK
6. Uruchom hello  **`azure network nsg subnet add`**  podsieci frontonu toohello polecenia toolink hello NSG.
...
1. Uruchom hello  **`azure network nsg create`**  toocreate polecenia grupy NSG.
   ...
   * **-l (lub --location)**. Region platformy Azure, której hello Nowa grupa NSG zostanie utworzona. W naszym scenariuszu *westus*.
   * **-n (lub --name)**. Nazwa hello Nowa grupa NSG. W naszym scenariuszu *frontonu NSG*.
2. Uruchom hello  **`azure network nsg rule create`**  toocreate polecenia regułę, która umożliwia dostęp tooport 1433 (SQL) z podsieci frontonu hello.
    ...
    Oczekiwane dane wyjściowe:
        ...
        data:    Priority                        : 100
        info:    network nsg rule create command OK
3. Uruchom hello  **`azure network nsg rule create`**  toocreate polecenia regułę, która odmawia dostępu toohello Internet.
    ...
    Oczekiwano putput:
        ...
        data:    Priority                        : 200
        info:    network nsg rule create command OK
4. Uruchom hello  **`azure network nsg subnet add`**  polecenia toolink hello NSG toohello ponownie kończyć podsieci.
