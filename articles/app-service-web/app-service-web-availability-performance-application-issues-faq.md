...
Aby ustalić, czy używasz lokalnej pamięci podręcznej, sprawdź aplikację usługi **ustawienia aplikacji** kartę. Jeśli w lokalnej pamięci podręcznej jest używany, aplikacji, ustawianie `WEBSITE_LOCAL_CACHE_OPTION` ma ustawioną wartość `Always`. Aby uzyskać więcej informacji o lokalnej pamięci podręcznej, zobacz [lokalnej pamięci podręcznej usługi aplikacji — omówienie](https://docs.microsoft.com/azure/app-service/app-service-local-cache).
