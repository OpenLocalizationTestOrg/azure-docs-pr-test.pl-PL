...
Zadania kodowania jest skojarzony z zasób wejściowy (lub zasoby) na którym chcesz tooperform niektóre zadania kodowania. Na przykład kodowanie MP4 pliku tooH.264 MP4 o adaptacyjnej szybkości bitowej ustawia; Tworzenie miniatur; Utwórz nakładki. Po zakończeniu zadania jest generowany zawartości wyjściowej.  zawartości wyjściowej Hello zawiera wideo, audio, miniatur, zawartości wyjściowej hello itp. zawiera także plik o metadane dotyczące zawartości wyjściowej hello. Witaj nazwę pliku XML metadanych hello ma hello następującego formatu: &lt;source_file_name&gt;_manifest.xml (na przykład BigBuckBunny_manifest.xml).  

Jeśli chcesz tooexamine hello metadanych plików, można utworzyć **SAS** lokalizator i pobierania hello komputera lokalnego tooyour pliku.  

W tym temacie omówiono hello elementów i typy hello schematu XML na które hello metada danych wyjściowych (&lt;source_file_name&gt;_manifest.xml) opiera się. Informacje hello zawierający metadane dotyczące zasobu wejściowego hello, zobacz [wejściowych metadanych](media-services-input-metadata-schema.md).  

> [!NOTE]
> Można znaleźć hello schematu pełny kod i przykład XML na powitania na końcu tego tematu.  
>
>

## <a name="AssetFiles "></a>Element główny AssetFiles
Kolekcja wpisów AssetFile hello zadania kodowania.  

### <a name="child-elements"></a>Elementy podrzędne
| Nazwa | Opis |
| --- | --- |
| **AssetFile**<br/><br/> minOccurs = maxOccurs "0" = "1" |[Elementu AssetFile](media-services-output-metadata-schema.md) będącego częścią hello AssetFiles kolekcji. |
...
| Nazwa | Typ | Opis |
| --- | --- | --- |
| **Nazwa**<br/><br/> Wymagane |**xs:String** |Nazwa pliku zasobu multimediów Hello. |
| **Rozmiar**<br/><br/> minInclusive = "0"<br/><br/> Wymagane |**xs:Long** |Rozmiar pliku zasobów hello w bajtach. |
| **Czas trwania**<br/><br/> Wymagane |**DURATION** |Czas trwania wstecz odtwarzania zawartości. |

### <a name="child-elements"></a>Elementy podrzędne
| Nazwa | Opis |
| --- | --- |
| **Źródeł** |Kolekcja plików multimedialnych źródła danych wejściowych /, który był przetwarzany w celu tooproduce to AssetFile. Aby uzyskać więcej informacji, zobacz [elementu źródła](media-services-output-metadata-schema.md). |
| **VideoTracks**<br/><br/> minOccurs = maxOccurs "0" = "1" |Każdy fizyczny AssetFile może zawierać w nim zero lub więcej ścieżek wideo przeplatana do formatu odpowiedniego kontenera. To jest kolekcja hello tych ścieżek wideo. Aby uzyskać więcej informacji, zobacz [VideoTracks elementu](media-services-output-metadata-schema.md). |
| **AudioTracks**<br/><br/> minOccurs = maxOccurs "0" = "1" |Każdy fizyczny AssetFile może zawierać w nim zero lub więcej ścieżek audio przeplatana do formatu odpowiedniego kontenera. To jest kolekcja hello tych ścieżek audio. Aby uzyskać więcej informacji, zobacz [AudioTracks elementu](media-services-output-metadata-schema.md). |

## <a name="Sources "></a>Element źródeł
Kolekcja plików multimedialnych źródła danych wejściowych /, który był przetwarzany w celu tooproduce to AssetFile.  
...
Każdy fizyczny AssetFile może zawierać w nim zero lub więcej ścieżek wideo przeplatana do formatu odpowiedniego kontenera. To jest kolekcja hello tych ścieżek wideo.  
...
| Nazwa | Opis |
| --- | --- |
| **VideoTrack**<br/><br/> minOccurs = maxOccurs "1" = "unbounded" |Określonej ścieżki wideo w nadrzędnym hello AssetFile. Aby uzyskać więcej informacji, zobacz [VideoTrack elementu](media-services-output-metadata-schema.md#VideoTrack). |

## <a name="VideoTrack"></a>VideoTrack element
Określonej ścieżki wideo w nadrzędnym hello AssetFile.  
...
| Nazwa | Typ | Opis |
| --- | --- | --- |
| **Identyfikator**<br/><br/> minInclusive = "0"<br/><br/> Wymagane |**xs:int** |Liczony od zera indeks tej ścieżki wideo. **Uwaga:** nie jest hello TrackID w pliku MP4. |
| **FourCC**<br/><br/> Wymagane |**xs:String** |Kodera-dekodera wideo FourCC kodu. |
| **Profil** |**xs:String** |H264 profilu (tylko odpowiednie tooH264 kodera-dekodera). |
| **Poziom** |**xs:String** |Poziom H264 (tylko odpowiednie tooH264 kodera-dekodera). |
...
| **Szybkość transmisji bitów**<br/><br/> minInclusive = "0"<br/><br/> Wymagane |**xs:int** |Średnia wideo szybkość transmisji bitów w kilobitów na sekundę, obliczoną z hello AssetFile. Zlicza tylko ładunek podstawowe strumienia hello, a nie obejmuje koszty pakietów hello. |
| **TargetBitrate**<br/><br/> minInclusive = "0"<br/><br/> Wymagane |**xs:int** |TARGET średnia szybkość transmisji bitów dla tej ścieżki wideo za pośrednictwem ustawień kodowania hello, w kilobitów na sekundę. |
...
Każdy fizyczny AssetFile może zawierać w nim zero lub więcej ścieżek audio przeplatana do formatu odpowiedniego kontenera. To jest kolekcja hello tych ścieżek audio.  
...
| Nazwa | Opis |
| --- | --- |
| **AudioTrack**<br/><br/> minOccurs = maxOccurs "1" = "unbounded" |Określonej ścieżki audio w nadrzędnym hello AssetFile. Aby uzyskać więcej informacji, zobacz [AudioTrack elementu](media-services-output-metadata-schema.md). |

## <a name="AudioTrack "></a>AudioTrack element
Określonej ścieżki audio w nadrzędnym hello AssetFile.  
...
| Nazwa | Typ | Opis |
| --- | --- | --- |
| **Identyfikator**<br/><br/> minInclusive = "0"<br/><br/> Wymagane |**xs:int** |Liczony od zera indeks tej ścieżki audio. **Uwaga:** nie jest hello TrackID w pliku MP4. |
...
| **Szybkość transmisji bitów**<br/><br/> minInclusive = "0"<br/><br/> Wymagane |**xs:int** |Średnia audio szybkość transmisji bitów w bitów na sekundę, obliczoną z hello AssetFile. Zlicza tylko ładunek podstawowe strumienia hello, a nie obejmuje koszty pakietów hello. |
| **BitsPerSample**<br/><br/> minInclusive = "0"<br/><br/> Wymagane |**xs:int** |Wpisz bitów na próbkę hello wFormatTag formatu. |
...
| **IntegratedLoudnessSpeechPercentage**<br/><br/> minInclusive = "0" maxInclusive = "100" |**xs:float** |Mowy zawartości przez program hello, jako wartość procentową. |
...
## <a name="xml"></a>Przykład pliku XML
 Hello poniżej znajduje się przykład pliku metadanych hello danych wyjściowych.  
