...
Microsoft Azure Media Services to usługa opartego na interfejsie REST oparty na OData v3. Można dodać zapytania, aktualizacji i usuwania jednostek w taki hello sam sposób, jak można w każdej usłudze OData. W razie potrzeby zostanie wywołana wyjątki. Aby uzyskać więcej informacji o OData, zobacz [dokumentacji protokołu Open Data Protocol](http://www.odata.org/documentation/).

W tym temacie opisano sposób toomanage jednostki usługi Azure Media Services z REST.
...
Każdy podmiot w usłudze Media Services jest dodawana tooan zestaw jednostek, takich jak zasoby, za pomocą żądania POST HTTP.
...
Witaj poniższy przykład powoduje pobranie listy wszystkich jednostek MediaProcessor.
...
Można również pobrać jednostki określonych lub wszystkich zestawów jednostek skojarzonych z wybraną jednostką, takich jak w hello następujące przykłady:
...
Witaj poniższy przykład zwraca tylko Właściwość State hello wszystkich zadań.
...
Witaj poniższy przykład zwraca wszystkie JobTemplates o nazwie hello "SampleTemplate."
...
> [!NOTE]
> Hello $rozwiń operacja jest nieobsługiwana w usłudze Media Services również hello nieobsługiwanej metody LINQ opisane w zagadnienia dotyczące LINQ (usługi danych WCF).
> 
> 

## <a name="enumerating-through-large-collections-of-entities"></a>Wyliczanie za pośrednictwem dużych kolekcjach jednostek
Podczas wykonywania zapytania jednostek, istnieje limit 1000 jednostek zwracane w tym samym czasie, ponieważ publiczny v2 REST ogranicza wyniki too1000 wyników zapytania. Użyj **pominąć** i **górnej** tooenumerate za pośrednictwem hello dużą kolekcję jednostek. 

powitania po przykładzie pokazano, jak toouse **pominąć** i **górnej** tooskip hello najpierw 2000 zadań i zadań obok 1000 hello get.  
...
W zależności od typu jednostki hello i jest w stanie hello można zaktualizować właściwości tego obiektu za pomocą poprawki żądania PUT, MERGE HTTP lub. Aby uzyskać więcej informacji o tych operacjach, zobacz [poprawki/PUT/MERGE](https://msdn.microsoft.com/library/dd541276.aspx).

Witaj poniższy przykład kodu pokazuje, jak tooupdate hello nazwa właściwości jednostki zasobów.
...
Jednostki mogą zostać usunięte w usłudze Media Services przy użyciu żądania DELETE HTTP. W zależności od podmiotu hello może być ważne hello kolejność, w którym usuwania jednostek. Na przykład jednostek, takich jak zasoby wymagają, aby odwołać (lub usuń) wszystkie lokalizatorów, które odwołują się do tego konkretnego zasobu przed usunięciem hello zasobów.

powitania po przykładzie pokazano, jak toodelete lokalizatora, który był używany tooupload pliku do magazynu obiektów blob.
