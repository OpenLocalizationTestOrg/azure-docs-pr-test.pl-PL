...
W tym temacie przedstawiono, jak ustawienie wstępne tooperform Zaawansowane kodowanie z Media Encoder Standard (rynkowej) przy użyciu niestandardowego. temat Hello używa .NET toocreate kodowania zadań i zadań, która wykonuje to zadanie.  

W tym temacie zobaczysz, jak hello toocustomize predefiniowanego wykonując [wielu transmisji bitów H264 720p](media-services-mes-preset-H264-Multiple-Bitrate-720p.md) hello wstępnie zdefiniowane i zmniejszenie liczby warstw. Witaj [ustawienia Dostosowywanie Media Encoder Standard](media-services-advanced-encoding-with-mes.md) temacie przedstawiono ustawienia niestandardowe, które mogą być używane tooperform zaawansowane zadania kodowania.
...
Zapisz hello JSON zdefiniowane w hello [wielu transmisji bitów H264 720p](media-services-mes-preset-H264-Multiple-Bitrate-720p.md) tematu w niektórych plików z rozszerzeniem JSON. Na przykład **CustomPreset_JSON.json**.

### <a name="customized-preset"></a>Dostosowane ustawienia wstępnego

Otwórz hello **CustomPreset_JSON.json** plików i usuń pierwsze trzy warstwy z **H264Layers** tak pliku wygląda następująco.
...
- Ładowanie ustawień niestandardowych JSON utworzony w poprzedniej sekcji hello powitalne. 
  
        // Load hello JSON from hello local file.
        string configuration = File.ReadAllText(fileName);  

- Dodaj zadania kodowania toohello zadań. 
