...
Możesz uruchomić hello narzędzia Import/Eksport Microsoft Azure z zadania importu hello kopii dziennika plików i hello narzędzie przekazywania hello brakujących plików (lub części pliku) tooyour Windows magazynu konta toocomplete hello zadaniem importu platformy Azure.  
...
|||  
|-|-|  
|**/ r:**< RepairFile\>|**Wymagane.** Plik naprawy toohello ścieżki, który śledzi postęp hello naprawy hello i pozwala tooresume naprawę przerwania. Każdy dysk musi mieć jeden i tylko jeden plik naprawy. Po uruchomieniu naprawy dla danego dysku przekazać hello ścieżki tooa naprawy pliku, które jeszcze nie istnieje. tooresume naprawę przerwania, należy przekazać w nazwie hello istniejącego pliku naprawy. zawsze należy określać Hello naprawy pliku odpowiedniego toohello dysk docelowy.|  
|**/ logdir:**< LogDirectory\>|**Opcjonalne.** Katalog dziennika Hello. Pełne pliki dziennika są zapisywane w katalogu toothis. Jeśli katalog dziennika nie jest określony, hello bieżący katalog jest używany jako katalog dziennika hello.|  
...
W hello poniższy przykład przedstawia plik dziennika kopii 64 KB części pliku został uszkodzony na dysku hello, który został wysłany dla zadania importu hello. Ponieważ jest to błąd tylko hello wskazane, hello pozostałe obiekty BLOB hello hello zadania zostały pomyślnie zaimportowane.  
...
Po upływie tego dziennika kopii toohello narzędzie importu/eksportu Azure narzędzia hello próbuje toofinish hello importu dla tego pliku przez skopiowanie zawartości brakuje hello hello sieci. Wyszukuje następujących hello powyższego przykładu hello oryginalny plik narzędzia hello `\animals\koala.jpg` w katalogach hello dwa `C:\Users\bob\Pictures` i `X:\BobBackup\photos`. Jeśli hello plików `C:\Users\bob\Pictures\animals\koala.jpg` istnieje hello Azure narzędzie importu/eksportu kopie hello Brak zakresu odpowiedniego obiektu blob danych toohello `http://bobmediaaccount.blob.core.windows.net/pictures/animals/koala.jpg`.  
...
Witaj `/PathMapFile` opcja umożliwia tooresolve te błędy. Można określić nazwę hello hello pliku, który zawiera listę plików, które hello narzędzia hello nie może zidentyfikować toocorrectly. Witaj następującego wiersza polecenia wypełnia `9WM35C2V_pathmap.txt`:  
