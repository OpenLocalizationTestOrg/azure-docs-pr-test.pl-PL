...
W tym artykule opisano, jak używasz [szablonu usługi Azure Resource Manager](../azure-resource-manager/resource-group-authoring-templates.md) tooconfigure Azure metryki alerty. Dzięki temu tooautomatically Konfigurowanie alertów dla zasobów, po utworzeniu tooensure poprawnie monitorowania wszystkich zasobów.

podstawowe kroki Hello są następujące:

1. Tworzenie szablonu w formacie JSON, który opisuje, jak toocreate hello alert.
...
Poniżej opisano sposób toocreate szablonu usługi Resource Manager, najpierw dla alertu samodzielnie, następnie alertu podczas tworzenia hello innego zasobu.

## <a name="resource-manager-template-for-a-metric-alert"></a>Szablon usługi Resource Manager metryki alertu
toocreate alert przy użyciu szablonu usługi Resource Manager, utwórz zasób typu `Microsoft.Insights/alertRules` i wypełnij wszystkie powiązane właściwości. Poniżej znajduje się szablon, który tworzy regułę alertu.
...
Wyjaśnienie hello schematu i właściwości dla reguły alertu [jest dostępnych tutaj](https://msdn.microsoft.com/library/azure/dn933805.aspx).

## <a name="resource-manager-template-for-a-resource-with-an-alert"></a>Szablon usługi Resource Manager dla zasobu z alertem
Alert w szablonie usługi Resource Manager najczęściej jest przydatne podczas tworzenia alertu podczas tworzenia zasobu. Na przykład może być tooensure który "Procesora % > 80" skonfigurować regułę za każdym razem, gdy wdrożenie maszyny wirtualnej. toodo, dodać reguły alertów hello jako zasób w tablicy zasobów hello szablonu maszyny Wirtualnej i Dodawanie zależności za pomocą hello `dependsOn` identyfikator zasobu właściwości toohello maszyny Wirtualnej. W tym miejscu jest pełny przykład, który tworzy Maszynę wirtualną systemu Windows i dodaje alert powiadamia Administratorzy subskrypcji, gdy hello wykorzystanie procesora CPU przekracza 80%.
...
* [Dowiedz się więcej o alertach](insights-receive-alert-notifications.md)
* [Dodaj ustawienia diagnostyki](monitoring-enable-diagnostic-logs-using-template.md) tooyour szablonu usługi Resource Manager
