...
Jeśli chodzi o ochronę konta weryfikacji dwuetapowej powinna być standardowe całej organizacji. Ta funkcja jest szczególnie ważne w przypadku konta z uprawnieniami administracyjnymi, które mają uprzywilejowany dostęp do zasobów. Z tego powodu firma Microsoft oferuje funkcje weryfikacji dwuetapowej podstawowe dla administratorów usługi Office 365 i Azure. Jeśli chcesz uaktualnić funkcje dla administratorów lub rozszerzyć weryfikację dwuetapową do pozostałej części użytkowników, możesz kupić Azure Multi-Factor Authentication. 

W tym artykule omówiono objaśniono różnicę między wersjami oferowane dla administratorów i pełną wersję usługi Azure MFA i określa, które funkcje są dostępne w każdym. Jeśli wszystko jest gotowe do wdrożenia pełną usługi Azure MFA oferty, kolejnych sekcjach opisano opcje wdrażania i jak Microsoft oblicza zużycia.
...
| Azure Multi-Factor Authentication | Często określany jako "pełnej" wersji, uwierzytelnianie wieloskładnikowe Azure oferuje najszerszym zestaw funkcji. Zapewnia dodatkowe opcje konfiguracji za pomocą [klasycznego portalu Azure](https://manage.windowsazure.com), zaawansowane raportowanie i obsługuje szeroką gamę lokalnych i aplikacji w chmurze. Uwierzytelnianie wieloskładnikowe platformy Azure znajduje się w usłudze Azure Active Directory Premium (planów P1 i P2) i Enterprise Mobility + Security (planów E3 i E5) i może być wdrożony albo [w chmurze lub lokalnie](multi-factor-authentication-get-started.md). |
...
Zakupu licencji, które obejmują usługi Azure Multi-Factor Authentication, takich jak Azure Active Directory Premium (P1 lub P2) lub pakietu Enterprise Mobility + Security (E3 lub E5) i przypisać je do użytkowników w usłudze Azure Active Directory. 
...
2. **Każde uwierzytelnianie** — w przypadku przedsiębiorstw, które chcesz włączyć weryfikację dwuetapową dla dużej grupy użytkowników, którzy rzadko potrzebują uwierzytelniania. Karta opiera się na liczbę żądań weryfikacji dwuetapowej odebranych przez usługi w chmurze Azure MFA, niezależnie od tego, czy te weryfikacji powiedzie się lub nie są dozwolone. Ta karta pojawia się w instrukcji użycia platformy Azure w pakietach 10 uwierzytelnienia i zgłaszania systemowi Commerce codziennie. 
