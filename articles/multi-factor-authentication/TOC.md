# Omówienie
## [Co to jest usługa Multi-Factor Authentication platformy Azure?](multi-factor-authentication.md)
## [Jak to działa](multi-factor-authentication-how-it-works.md)
## [Jak tooget usługi Azure MFA](multi-factor-authentication-versions-plans.md)

# Rozpoczęcie pracy
## [Wybierz miejsce toodeploy](multi-factor-authentication-get-started.md)
## [Uwierzytelniania MFA w chmurze hello](multi-factor-authentication-get-started-cloud.md)
## [Lokalna usługa MFA](multi-factor-authentication-get-started-server.md)
## [Usługa MFA dla użytkowników usługi O365](https://support.office.com/article/Plan-for-multi-factor-authentication-for-Office-365-Deployments-043807b2-21db-4d5c-b430-c8a6dee0e6ba)
## [Najlepsze rozwiązania dotyczące zabezpieczeń](multi-factor-authentication-security-best-practices.md)

# Instrukcje
## Wdrażanie i używanie
### [Konfigurowanie ustawień](multi-factor-authentication-whats-next.md)
### [Raporty w usłudze MFA](multi-factor-authentication-manage-reports.md)
### [Używanie istniejących serwerów zasad sieciowych](multi-factor-authentication-nps-extension.md)
#### [Advanced configuration for NPS extension (Konfiguracja zaawansowana rozszerzenia serwera NPS)](nps-extension-advanced-configuration.md)
### [Tworzenie dostawcy uwierzytelniania](multi-factor-authentication-get-started-auth-provider.md)
### Wdrażanie lokalnego uwierzytelniania wieloskładnikowego
#### [Zainstaluj portal użytkowników hello](multi-factor-authentication-get-started-portal.md)
#### [Zaawansowane konfiguracje sieci VPN](multi-factor-authentication-advanced-vpn-configurations.md)
#### [Usługa sieci Web aplikacji mobilnej](multi-factor-authentication-get-started-server-webservice.md)
#### [Konfigurowanie wysokiej dostępności](mfa-server-high-availability.md)
#### [Uwierzytelnianie systemu Windows](multi-factor-authentication-get-started-server-windows.md)
#### [Uaktualnianie serwera MFA](multi-factor-authentication-server-upgrade.md)
#### [Uaktualnianie z narzędzia PhoneFactor](multi-factor-authentication-get-started-server-upgrade.md)

## Zarządzanie użytkownikami
### [Włączanie lub wyłączanie usługi MFA](multi-factor-authentication-get-started-user-states.md)
### [Ustawienia użytkownika i urządzenia](multi-factor-authentication-manage-users-and-devices.md)
### [Podręcznik użytkownika uwierzytelniania wieloskładnikowego](./end-user/multi-factor-authentication-end-user.md)

## Integracja
### [Brama pulpitu zdalnego](nps-extension-remote-desktop-gateway.md)
### [VPN](nps-extension-vpn.md)
### Integrowanie lokalnego uwierzytelniania wieloskładnikowego
#### [Usługi Federation Services](multi-factor-authentication-get-started-adfs.md)
##### [Korzystanie z usług AD FS](multi-factor-authentication-get-started-adfs-cloud.md)
##### [Korzystanie z usług AD FS 2.0](multi-factor-authentication-get-started-adfs-adfs2.md)
##### [Korzystanie z usług AD FS w systemie Windows Server 2012 R2](multi-factor-authentication-get-started-adfs-w2k12.md)
#### Katalogi
##### [Uwierzytelnianie LDAP](multi-factor-authentication-get-started-server-ldap.md)
##### [Uwierzytelnianie za pomocą protokołu RADIUS](multi-factor-authentication-get-started-server-radius.md)
##### [Usługa Active Directory](multi-factor-authentication-get-started-server-dirint.md)
#### [Aplikacje sieci Web usługi IIS](multi-factor-authentication-get-started-server-iis.md)
#### [Brama pulpitu zdalnego](multi-factor-authentication-get-started-server-rdg.md)

## Programowanie
### [Stosowanie w aplikacjach niestandardowych (SDK)](multi-factor-authentication-sdk.md)

## Rozwiązywanie problemów
### [Często zadawane pytania](multi-factor-authentication-faq.md)
### [Zadaj pytanie](https://social.msdn.microsoft.com/Forums/newthread?category=windowsazureplatform&forum=windowsazureactiveauthentication&prof=required)
### [Błędy rozszerzenia serwera NPS](multi-factor-authentication-nps-errors.md)

# Zasoby
## [Harmonogram działania dla platformy Azure](https://azure.microsoft.com/roadmap/?category=security-identity)
## [Kalkulator cen](https://azure.microsoft.com/pricing/calculator/)
