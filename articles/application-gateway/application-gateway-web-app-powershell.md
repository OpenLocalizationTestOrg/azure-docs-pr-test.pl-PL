...
Brama aplikacji pozwala toohave aplikacji sieci Web platformy Azure lub innej usługi wielodostępne jako element członkowski puli zaplecza. W tym artykule tooyou informacje tooconfigure aplikacji sieci web platformy Azure z bramy aplikacji. Witaj pierwszym przykładzie pokazano, jak tooconfigure istniejących toouse brama aplikacji w aplikacji sieci web jako element członkowski puli zaplecza. Witaj drugim przykładzie pokazano, jak toocreate nową bramę aplikacji z aplikacją sieci web jako element członkowski puli zaplecza.
...
Witaj poniższy przykład umożliwia dodanie aplikacji sieci web jako pula zaplecza elementu członkowskiego tooan istniejących aplikacji bramy. Witaj zarówno przełącznika `-PickHostNamefromBackendHttpSettings`na powitania badania konfiguracji i `-PickHostNameFromBackendAddress` HTTP zaplecza hello ustawienia musi być wprowadzona w kolejności dla toowork aplikacji sieci web.
...
W tym scenariuszu wdraża aplikację sieci web ASP.NET hello uzyskiwanie uruchomiono witryny sieci Web i bramy aplikacji.
...
Dowiedz się, jak przekierowanie tooconfigure odwiedzając: [Konfigurowanie przekierowywania dla bramy aplikacji przy użyciu programu PowerShell](application-gateway-configure-redirect-powershell.md).
