---
title: "Przekierowanie aaaConfigure bramy aplikacji Azure — PowerShell | Dokumentacja firmy Microsoft"
description: "Ta strona umożliwia przekierowanie tooconfigure scenariusze dla bramy aplikacji przy użyciu programu PowerShell"
...
---
# <a name="configure-redirection-on-application-gateway-with-powershell"></a><span data-ttu-id="1fdf6-103">Konfigurowanie przekierowania dla usługi Application Gateway przy użyciu programu PowerShell</span><span class="sxs-lookup"><span data-stu-id="1fdf6-103">Configure redirection on Application Gateway with PowerShell</span></span>

<span data-ttu-id="1fdf6-104">Brama aplikacji w obsługuje ruch tooredirect możliwości hello na podstawie zdefiniowanych konfiguracji.</span><span class="sxs-lookup"><span data-stu-id="1fdf6-104">Application gateway supports hello ability tooredirect traffic based on a defined configuration.</span></span> <span data-ttu-id="1fdf6-105">więcej informacji na temat przekierowania w ogólne odwiedziny toolearn [omówienie przekierowania bramy aplikacji](application-gateway-redirect-overview.md).</span><span class="sxs-lookup"><span data-stu-id="1fdf6-105">toolearn more about redirection in general visit, [Application Gateway redirect overview](application-gateway-redirect-overview.md).</span></span> <span data-ttu-id="1fdf6-106">W tym artykule przedstawiono przykłady tooHTTPS przekierowywania, opartych na ścieżce przekierowuje przekierowuje obejmujący wiele lokacji i przekierowuje tooexternal witryn.</span><span class="sxs-lookup"><span data-stu-id="1fdf6-106">This article provides examples of HTTP tooHTTPS redirection, path based redirects, multi-site redirects, and redirects tooexternal sites.</span></span>

## <a name="http-toohttps-redirect-on-an-existing-application-gateway"></a><span data-ttu-id="1fdf6-107">Przekierowywanie HTTP tooHTTPS na istniejącą bramę aplikacji</span><span class="sxs-lookup"><span data-stu-id="1fdf6-107">HTTP tooHTTPS redirect on an existing application gateway</span></span>

<span data-ttu-id="1fdf6-108">Witaj poniższy przykład tworzy nowy odbiornik HTTP na porcie 80 tooredirect żądań toohello istniejący odbiornik na porcie 443.</span><span class="sxs-lookup"><span data-stu-id="1fdf6-108">hello following example creates a new HTTP listener on port 80 tooredirect requests toohello existing listener on port 443.</span></span>
...
<span data-ttu-id="1fdf6-109">Po zaktualizowaniu bramy aplikacji hello przetestuj nowe odbiornika hello na porcie 80.</span><span class="sxs-lookup"><span data-stu-id="1fdf6-109">Once hello application gateway is updated, test hello new listener on port 80.</span></span>  <span data-ttu-id="1fdf6-110">Witaj poniższy przykład przedstawia użycie `curl` test hello przekierowania.</span><span class="sxs-lookup"><span data-stu-id="1fdf6-110">hello following example shows using `curl` test hello redirection.</span></span>
...
## <a name="path-based-redirect"></a><span data-ttu-id="1fdf6-111">Przekierowanie oparte na ścieżce</span><span class="sxs-lookup"><span data-stu-id="1fdf6-111">Path based redirect</span></span>

<span data-ttu-id="1fdf6-112">Hello poniższy przykład tworzy nowy i reguły ścieżki, oparte na istniejącą bramę aplikacji, który przekierowuje tylko ruch spotkania hello określonej ścieżki adresu url.</span><span class="sxs-lookup"><span data-stu-id="1fdf6-112">hello following example creates a new listener and a path based rule on an existing application gateway that redirects only traffic meeting hello specific url path.</span></span>
...
## <a name="multi-site-redirect"></a><span data-ttu-id="1fdf6-113">Przekierowanie obejmujące wiele lokacji</span><span class="sxs-lookup"><span data-stu-id="1fdf6-113">Multi-site redirect</span></span>

<span data-ttu-id="1fdf6-114">Witaj poniższy przykład tworzy nową bramę aplikacji z 2 odbiorników obejmujący wiele lokacji na porcie 80.</span><span class="sxs-lookup"><span data-stu-id="1fdf6-114">hello following example creates a new application gateway with 2 multi-site listeners on port 80.</span></span> <span data-ttu-id="1fdf6-115">odbiorniki Hello są adatum.com i adatum.org. Ruch tooredirect z adatum.org tooadatum.com jest tworzona reguła przekierowania. Dodatkowa konfiguracja jest wymagana do konfigurowania CNAME aliasy toohello aplikacji publiczny adres IP bramy, aby uzyskać więcej informacji dotyczących delegowania tooAzure Twojego domeny DNS oraz utworzenie CNAME rekordy w domenie można znaleźć [Delegowanie domeny tooAzure DNS](../dns/dns-delegate-domain-azure-dns.md).</span><span class="sxs-lookup"><span data-stu-id="1fdf6-115">hello listeners are for adatum.com and adatum.org. A redirect rule is created tooredirect traffic from adatum.org tooadatum.com. Additional configuration is required for configuring CNAME aliases toohello application gateway public IP address, for more information on delegating your domain tooAzure DNS and creating CNAME records for your domain visit, [Delegate a domain tooAzure DNS](../dns/dns-delegate-domain-azure-dns.md).</span></span>
...
## <a name="redirect-tooan-external-site"></a><span data-ttu-id="1fdf6-116">Przekierowanie tooan zewnętrznej witryny</span><span class="sxs-lookup"><span data-stu-id="1fdf6-116">Redirect tooan external site</span></span>

<span data-ttu-id="1fdf6-117">Witaj poniższy przykład przekierowuje ruch do hello aplikacji bramy tooan zewnętrznej witryny sieci Web (bing.com).</span><span class="sxs-lookup"><span data-stu-id="1fdf6-117">hello following example redirects traffic coming into hello application gateway tooan external website (bing.com).</span></span> <span data-ttu-id="1fdf6-118">Korzystając z hello `TargetUrl` hello przełącznika `IncludePath` i `IncludeQuery` przełączniki są niedozwolone.</span><span class="sxs-lookup"><span data-stu-id="1fdf6-118">When using hello `TargetUrl` switch hello `IncludePath` and `IncludeQuery` switches are not allowed.</span></span>
...
## <a name="next-steps"></a><span data-ttu-id="1fdf6-119">Następne kroki</span><span class="sxs-lookup"><span data-stu-id="1fdf6-119">Next steps</span></span>

<span data-ttu-id="1fdf6-120">Odwiedź stronę [konfigurować tooend końcowych SSL bramy aplikacji przy użyciu programu PowerShell](application-gateway-end-to-end-ssl-powershell.md)</span><span class="sxs-lookup"><span data-stu-id="1fdf6-120">Visit [configure end tooend SSL with application gateway using PowerShell](application-gateway-end-to-end-ssl-powershell.md)</span></span>
