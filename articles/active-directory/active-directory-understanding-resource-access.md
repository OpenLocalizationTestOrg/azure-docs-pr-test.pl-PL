...
> [!IMPORTANT]
> Firma Microsoft zaleca zarządzanie usługą Azure AD przy użyciu [centrum administracyjnego usługi Azure AD](https://aad.portal.azure.com) w witrynie Azure Portal zamiast korzystania z klasycznej witryny Azure Portal przywołanej w niniejszym artykule. Azure portal udostępnia [kontroli dostępu opartej na rolach](role-based-access-control-configure.md) tak zasobów platformy Azure można zarządzać dokładniej.
> 
> 

W październiku 2013 r. klasycznego portalu Azure i interfejsów API usługi Service Management zostały zintegrowane z usługi Azure Active Directory w celu przygotowawczych poprawy środowisko użytkownika w celu zarządzania dostępem do zasobów platformy Azure. Azure Active Directory zawiera już dużą możliwości, takie jak zarządzanie użytkownikami, synchronizacja katalogu lokalnego uwierzytelniania wieloskładnikowego i kontroli dostępu aplikacji. Oczywiście te powinny również dostępne do zarządzania zasobami Azure ogólnego.
