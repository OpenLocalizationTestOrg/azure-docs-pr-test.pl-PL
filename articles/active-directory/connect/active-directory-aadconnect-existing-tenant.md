...
Użytkownicy zazwyczaj mają userPrincipalName jak proxyAddresses, dzięki łatwy do dopasowania. Jednak inne obiekty, takie jak grupy zabezpieczeń nie są one. W takim przypadku może wyłącznie odpowiadać na dopasowanie twardych przy użyciu sourceAnchor. SourceAnchor jest zawsze Base64 przekonwertować **objectGUID** lokalnymi, więc należy zaktualizować wartość w usłudze Azure AD, gdy są potrzebne dwa obiekty do dopasowania. SourceAnchor/nazwę immutableID mogą być aktualizowane tylko przy użyciu programu PowerShell, a nie za pośrednictwem portali.
