...
Użytkownik produktu Workday inicjowania obsługi przepływów pracy obsługiwanych przez usługę inicjowania obsługi administracyjnej użytkowników usługi Azure AD umożliwia automatyzację następujących zasobów ludzkich i scenariusze zarządzania cyklem życia tożsamości:

* **Wynajem nowi pracownicy** — po dodaniu nowych pracowników do produktu Workday, konto użytkownika zostanie automatycznie utworzone w usłudze Active Directory, usługi Azure Active Directory i opcjonalnie usługi Office 365 i [innych aplikacji SaaS obsługiwane przez usługę Azure AD](active-directory-saas-app-provisioning.md), zapisu obu adres e-mail do produktu Workday.
...
> [!NOTE]
> Jeśli dzierżawy usługi Azure AD znajduje się w Europie, zobacz [znane problemy](#known-issues) poniższej sekcji.
...
Po zakończeniu części 1-3, można uruchomić usługę inicjowania obsługi administracyjnej w portalu zarządzania Azure.
...
4. Synchronizacja poszczególnych zdarzeń, takich jak co użytkownicy są trwa odczytu z produktem Workday, a następnie dodane lub zaktualizowane w usłudze Active Directory, można wyświetlić w **dzienników inspekcji** kartę. **[W podręczniku obsługi administracyjnej raportowania szczegółowe instrukcje na temat sposobu czytanie dzienników inspekcji](active-directory-saas-provisioning-reporting.md)**

5.  Dziennik aplikacji systemu Windows na maszynie agenta, zostaną wyświetlone wszystkie operacje wykonane przez agenta.
