...
Azure opartej na rolach kontroli dostępu (RBAC) jest dostarczany z hello następujące wbudowane role, które można przypisać toousers, grup i usług. Nie można zmodyfikować hello definicje ról wbudowanych. Można jednak utworzyć [niestandardowych ról w Azure RBAC](role-based-access-control-custom-roles.md) toofit hello specyficznych potrzeb Twojej organizacji.

## <a name="roles-in-azure"></a>Role na platformie Azure
Witaj Poniższa tabela zawiera krótkie opisy hello wbudowane role. Kliknij przycisk hello roli nazwa toosee hello szczegółową listę **akcje** i **notactions** hello roli. Witaj **akcje** właściwość określa hello dozwolonych akcji na zasobów platformy Azure. Ciągi akcji można używać symboli wieloznacznych. Witaj **notactions** właściwość określa hello akcje, które są wykluczone z hello dozwolonych akcji.

Akcja Hello Określa jaki typ operacji, które można wykonywać na typ zasobu. Na przykład:
- **Zapis** umożliwia tooperform możesz PUT, POST, zastosować poprawki i operacji usunięcia.
- **Odczyt** pozwala tooperform operacji GET.

W tym artykule opisano tylko hello różne role, które istnieją już dzisiaj. Po przypisaniu roli użytkownika tooa, można ograniczyć hello dozwolone przez zdefiniowanie zakresu dalsze akcje. Jest to przydatne, jeśli toomake ktoś współautora witryny sieci Web, ale tylko dla jednej grupy zasobów.

> [!NOTE]
> stale ewoluuje Hello definicje ról platformy Azure. W tym artykule jest przechowywana jako zapasowych toodate, jak to możliwe, ale zawsze można znaleźć hello najnowsze definicje ról w programie Azure PowerShell. Użyj hello [Get-AzureRmRoleDefinition](/powershell/module/azurerm.resources/get-azurermroledefinition) toolist polecenia cmdlet wszystkich bieżących ról. Użytkownik może zajrzyj dostęp tooa konkretnej roli za pomocą `(get-azurermroledefinition "<role name>").actions` lub `(get-azurermroledefinition "<role name>").notactions` ma zastosowanie. Użyj [Get-AzureRmProviderOperation](/powershell/module/azurerm.resources/get-azurermprovideroperation) operacji toolist dostawców określonych zasobów platformy Azure.


| Nazwa roli | Opis |
| --- | --- |
| [Interfejs API zarządzania usługi współautora](#api-management-service-contributor) |Można zarządzać hello interfejsów API i usługi Zarządzanie interfejsami API |
| [Rola operatora usługi zarządzania interfejsu API](#api-management-service-operator-role) | Można zarządzać usługi API Management, ale hello interfejsów API się |
| [Interfejs API zarządzania usługi czytnika roli](#api-management-service-reader-role) | Usługa zarządzania tooAPI dostęp tylko do odczytu i interfejsów API |
...
| [Operator automatyzacji](#automation-operator) |Toostart stanie zatrzymania, wstrzymywanie i wznawianie prac |
...
| [Menedżera zabezpieczeń programu SQL](#sql-security-manager) |Można zarządzać zasadami związanych z zabezpieczeniami hello serwerów SQL i baz danych |
...
| [Administrator dostępu użytkowników](#user-access-administrator) |Zarządzanie zasobami tooAzure dostępu użytkownika |
| [Współautor klasyczne maszyny wirtualnej](#classic-virtual-machine-contributor) |Można zarządzać klasycznych maszyn wirtualnych, ale nie hello sieci wirtualnej lub toowhich konta magazynu, które są połączone |
| [Współautor maszyny wirtualnej](#virtual-machine-contributor) |Można zarządzać maszyn wirtualnych, ale nie hello wirtualnych sieci lub magazynu konta toowhich są one podłączone |
...
| [Współautor witryny sieci Web](#website-contributor) |Zarządzaj witrynami sieci Web, ale nie hello toowhich planów sieci web, które są połączone |

## <a name="role-permissions"></a>Uprawnienia roli
Witaj poniższych tabelach opisano hello określone uprawnienia, biorąc pod uwagę tooeach rolę. Może to obejmować **akcje**, które dają uprawnienia, i **NotActions**, które ograniczają.
...
| Microsoft.ResourceHealth/availabilityStatuses/read |Przeczytaj kondycji hello zasobów |
...
| Microsoft.ApiManagement/Service/backup/action | Wykonaj kopię zapasową usługi interfejsu API zarządzania toohello określonego kontenera użytkownika podane konto magazynu |
...
| Microsoft.ApiManagement/Service/restore/action | Przywracanie usługi interfejsu API zarządzania z określonego kontenera hello użytkownika podane konto magazynu |
...
Toostart stanie zatrzymania, wstrzymywanie i wznawianie prac
...
Można zarządzać wszystkie akcje zarządzania kopiami zapasowymi, z wyjątkiem Tworzenie magazynu usług odzyskiwania i podając tooothers dostępu
...
| Microsoft.RecoveryServices/Vaults/backupManagementMetaData/* | Tworzenie i zarządzanie nimi meta zarządzania toobackup powiązane dane |
...
| Microsoft.RecoveryServices/Vaults/certificates/* | Tworzenie i zarządzanie nimi toobackup pokrewne certyfikaty w magazynie usług odzyskiwania |
| Microsoft.RecoveryServices/Vaults/extendedInformation/* | Tworzenie i zarządzanie nimi rozszerzone informacje dotyczące toovault |
...
Można zarządzać wszystkie akcje zarządzania kopiami zapasowymi, z wyjątkiem tworzenia magazynów, usuwanie kopii zapasowej i podając tooothers dostępu
...
| Microsoft.RecoveryServices/Vaults/backupJobs/* | Tworzenie i zarządzanie nimi zadania tworzenia kopii zapasowej |
| Microsoft.RecoveryServices/Vaults/backupJobsExport/action | Eksportowanie zadań tworzenia kopii zapasowej do programu excel |
| Microsoft.RecoveryServices/Vaults/backupManagementMetaData/read | Odczytu metadane dotyczące zarządzania toobackup |
| Microsoft.RecoveryServices/Vaults/backupOperationResults/* | Utwórz i Zarządzaj wynikami operacji zarządzania kopiami zapasowymi |
...
| Microsoft.RecoveryServices/Vaults/extendedInformation/read | Odczyt rozszerzone informacje dotyczące toovault |
| Microsoft.RecoveryServices/Vaults/extendedInformation/write | Zapis rozszerzone informacje dotyczące toovault |
| Microsoft.RecoveryServices/Vaults/read | Magazyny usług odzyskiwania do odczytu |
| Microsoft.RecoveryServices/Vaults/refreshContainers/* | Zarządzanie pobieranie operacji wykrywania nowo utworzony kontenerów |
| Microsoft.RecoveryServices/Vaults/registeredIdentities/operationResults/read | Wyniki operacji odczytu wykonywane na elementach zarejestrowanej hello magazynu |
| Microsoft.RecoveryServices/Vaults/registeredIdentities/read | Odczytuj elementy zarejestrowanych hello magazynu |
| Microsoft.RecoveryServices/Vaults/registeredIdentities/write | Zapis toovault zarejestrowanych elementów |
| Microsoft.RecoveryServices/Vaults/usages/read | Użycie odczytu hello magazyn usług odzyskiwania |
| Microsoft.Resources/deployments/* | Tworzenie i zarządzanie nimi wdrożenia grupy zasobów |
| Microsoft.Resources/subscriptions/resourceGroups/read | Odczytanie grup zasobów |
| Microsoft.Storage/storageAccounts/read | Przeczytaj kont magazynu |
| Microsoft.Support/* | Tworzenie i zarządzanie biletami pomocy technicznej |
...
| --- | --- |
| Microsoft.Authorization/*/read |Role odczytu i przypisania roli |
| Microsoft.Compute/availabilitySets/read |Właściwości odczytu hello zestawów dostępności |
| Microsoft.Compute/virtualMachines/*/read |Odczytywanie hello właściwości maszyny wirtualnej (rozmiary maszyn wirtualnych stanu w czasie wykonywania, rozszerzeń maszyny Wirtualnej, itp.) |
| Microsoft.Compute/virtualMachines/deallocate/action |Cofnij Przydział maszyny wirtualne |
| Microsoft.Compute/virtualMachines/read |Odczyt hello właściwości maszyny wirtualnej |
...
| Microsoft.DevTestLab/*/read |Właściwości odczytu hello laboratorium |
...
| Microsoft.Network/networkInterfaces/*/read |Odczytywanie hello właściwości interfejsu sieciowego (na przykład wszystkich usług równoważenia obciążenia hello ten interfejs sieciowy hello jest częścią) |
| Microsoft.Network/networkInterfaces/join/action |Dołącz do interfejsu sieciowego tooa maszyny wirtualnej |
...
| Microsoft.Network/publicIPAddresses/*/read |Właściwości odczytu hello publicznego adresu IP |
...
| Microsoft.ResourceHealth/availabilityStatuses/read |Przeczytaj kondycji hello hello zasobów |
| Microsoft.Resources/deployments/\* |Tworzenie i zarządzanie nimi wdrożenia grupy zasobów |
| Microsoft.Resources/subscriptions/resourceGroups/read |Odczytanie grup zasobów |
| Microsoft.Support/\* |Tworzenie i zarządzanie biletami pomocy technicznej |
...
| Microsoft.Insights/eventtypes/* |Wyświetl zdarzenia dziennika aktywności (zdarzeń zarządzania) w ramach subskrypcji. To uprawnienie jest zastosowanie tooboth dostęp programistyczny i portalu toohello dziennik aktywności. |
| Microsoft.Insights/LogDefinitions/* |To uprawnienie jest wymagane dla użytkowników, którzy wymagają dostępu tooActivity dzienniki za pośrednictwem portalu hello. Lista kategorii dziennika w dzienniku aktywności. |
...
| Microsoft.Insights/Register/Action |Zarejestruj dostawcę elemencie Microsoft.Insights hello. |
...
Można zarządzać wszystkie akcje zarządzania odzyskiwania lokacji, z wyjątkiem Tworzenie magazynu usług odzyskiwania i przypisywanie użytkowników tooother prawa dostępu
...
| Microsoft.Insights/alertRules/* | Tworzenie i zarządzanie nimi reguły alertów |
| Microsoft.Network/virtualNetworks/read | Sieci wirtualne odczytu |
| Microsoft.RecoveryServices/Vaults/certificates/write | Certyfikat poświadczeń magazynu hello aktualizacji |
| Microsoft.RecoveryServices/Vaults/extendedInformation/* | Tworzenie i zarządzanie nimi rozszerzone informacje dotyczące toovault |
| Microsoft.RecoveryServices/Vaults/monitoringAlerts/*  | Odczyt alertów dla magazynu usług odzyskiwania hello |
...
Można trybu Failover i powrotu po awarii, ale nie można wykonywać inne akcje zarządzania usługi Site Recovery lub przypisywanie dostępu użytkowników tooother
...
| Microsoft.RecoveryServices/vaults/replicationEvents/read | Przeczytaj zdarzenia replikacji |
| Microsoft.RecoveryServices/vaults/replicationFabrics/checkConsistency/action | Sprawdzanie spójności sieci szkieletowych hello |
...
| Microsoft.RecoveryServices/vaults/replicationFabrics/ replicationProtectionContainers/replicationProtectedItems/updateMobilityService/działania | Aktualizacja usługi mobilności hello |
...
| Microsoft.Authorization/*/read | Role odczytu i przypisania ról |
| Microsoft.RecoveryServices/Vaults/extendedInformation/read  | Odczyt rozszerzone informacje dotyczące toovault |
| Microsoft.RecoveryServices/Vaults/monitoringAlerts/read  | Odczyt alertów dla magazynu usług odzyskiwania hello |
| Microsoft.RecoveryServices/Vaults/monitoringConfigurations/ notificationConfiguration/Odczyt  | Konfiguracja powiadomień w magazynie usług odzyskiwania odczytu |
| Microsoft.RecoveryServices/Vaults/read  | Magazyny usług odzyskiwania odczytu |
| Microsoft.RecoveryServices/Vaults/refreshContainers/read  | Zarządzanie pobieranie operacji wykrywania nowo utworzony kontenerów |
| Microsoft.RecoveryServices/Vaults/registeredIdentities/operationResults/read  | Odczytywanie stan operacji i wynik operacji zostało przesłane |
...
Można zarządzać zasadami związanych z zabezpieczeniami hello serwerów SQL i baz danych
...
Można zarządzać kontami magazynu, ale nie ma dostępu toothem.
...
Można tworzyć i zarządzanie biletami pomocy technicznej w zakresie subskrypcji hello
...
Zarządzanie zasobami tooAzure dostępu użytkownika
...
Można zarządzać klasycznych maszyn wirtualnych, ale nie hello sieci wirtualnej lub toowhich konta magazynu, które są połączone
...
Można zarządzać, ale nie hello wirtualnych sieci lub magazynu konta toowhich gdy są połączeni maszyny wirtualne
...
Można zarządzać witryn sieci Web, ale nie hello toowhich planów sieci web, które są połączone
...
| Microsoft.Web/listSitesAssignedToHostName/read |Witryny odczytu przypisane tooa nazwy hosta |
| Microsoft.Web/serverFarms/join/action |Dołącz do farmy serwerów |
| Microsoft.Web/serverFarms/read |Przeczytaj farmy serwerów |
| Microsoft.Web/sites/* |Tworzenie i zarządzanie nimi witryn sieci Web (Tworzenie witryny wymaga także toohello uprawnienia zapisu skojarzone planu usługi App Service) |

## <a name="see-also"></a>Zobacz też
* [Kontrola dostępu oparta na rolach](role-based-access-control-configure.md): rozpoczynanie pracy z RBAC w hello portalu Azure.
