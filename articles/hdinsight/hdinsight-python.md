...
1. Użyj `hive` polecenie, aby uruchomić powłokę programu hive. Powinny pojawić się `hive>` Monituj po załadowaniu powłoki.

2. Wpisz poniższe zapytanie na `hive>` wiersza:
...
1. Użyj `pig` polecenie, aby uruchomić powłoki. Zostanie wyświetlony `grunt>` Monituj po załadowaniu powłoki.
...
5. Raz w edytorze, usuń znaczniki komentarza następujący wiersz przez usunięcie `#` znaku od początku wiersza:
    ...
    Po dokonaniu zmian, użyj klawiszy Ctrl + X, aby zakończyć działanie edytora. Wybierz Y, a następnie wprowadź, aby zapisać zmiany.
