...
* (Opcjonalnie) **Programu visual Studio**: zestaw Azure SDK 2.5.1 lub nowszej i narzędzi Data Lake Tools dla programu Visual Studio. Aby uzyskać więcej informacji, zobacz [rozpoczęcie pracy przy użyciu narzędzi Data Lake Tools dla programu Visual Studio](hdinsight-hadoop-visual-studio-tools-get-started.md).
...
1. Jeśli nie został już zainstalowany najnowszą wersję narzędzi Data Lake Tools dla programu Visual Studio, zobacz [rozpoczęcie pracy przy użyciu narzędzi Data Lake Tools dla programu Visual Studio](hdinsight-hadoop-visual-studio-tools-get-started.md).
...
2. Użyj następującego polecenia, aby uruchomić przykładową topologię:
    ...
    To polecenie uruchamia przykładową topologię WordCount w klastrze. Ta topologia losowo wygenerowanie zdań i zliczenie wystąpień poszczególnych wyrazów w zdaniach.
...
* **Z sieci Ambari Web**: Wybierz **usług** w górnej części strony, następnie wybierz **Storm**. Z **Podsumowanie** wybierz opcję **serwera interfejsu użytkownika Storm**. Nazwa FQDN węzła, który działa interfejsu użytkownika platformy Storm i interfejsu API REST jest w górnej części strony.
...
Można używać z wewnątrz klastra lub maszyn wirtualnych w tej samej sieci wirtualnej Azure, co klaster może być tylko informacje zwrócone z interfejsu API REST. Na przykład w pełni kwalifikowaną nazwę (FQDN) zwróciła dozorcy serwerów jest nie być dostępny z Internetu.

## <a name="next-steps"></a>Następne kroki

Teraz, kiedy znasz już sposobu wdrażania i monitorowania topologii za pomocą pulpitu nawigacyjnego Storm, Dowiedz się, jak [topologie oparte na opracowanie Java za pomocą programu Maven](hdinsight-storm-develop-java-topology.md).
