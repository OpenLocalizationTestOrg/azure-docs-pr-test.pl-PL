...
Dowiedz się więcej o hello Najważniejsze problemy i rozwiązania ich podczas pracy z ładunków systemu Distributed plików Hadoop (HDFS) w Apache Ambari.

## <a name="how-do-i-access-local-hdfs-from-inside-a-cluster"></a>Jak dostęp hello lokalnego systemu plików HDFS z wewnątrz klastra

### <a name="issue"></a>Problem

Witaj dostępu lokalnego systemu plików HDFS z hello kod wiersza i aplikacji zamiast polecenia przy użyciu magazynu obiektów Blob platformy Azure lub usługi Azure Data Lake Store z wewnątrz hello klastra usługi HDInsight.   

### <a name="resolution-steps"></a>Kroki rozwiązania

1. W wierszu polecenia hello użyj `hdfs dfs -D "fs.default.name=hdfs://mycluster/" ...` jako literału, tak jak hello następujące polecenie:
...
2. Kod źródłowy, użyj funkcji hello URI `hdfs://mycluster/` jako literału, tak jak po Przykładowa aplikacja hello:
...
3. Uruchom hello skompilowany plik JAR (na przykład plik o nazwie `java-unit-tests-1.0.jar`) na powitania HDInsight klaster z hello następujące polecenie:
...
Witaj, który w lokalnym systemie plików HDFS jest zablokowana w trybie awaryjnym w klastrze usługi HDInsight hello.   

### <a name="detailed-description"></a>Szczegółowy opis

Błąd występuje, gdy uruchom następujące polecenia w systemie plików HDFS hello:

```apache
hdfs dfs -D "fs.default.name=hdfs://mycluster/" -mkdir /temp
```

Zostanie wyświetlony następujący błąd podczas wykonywania polecenia hello hello:
...
1. Uzyskiwanie hello stan systemu plików HDFS w klastrze usługi HDInsight hello przy użyciu hello następującego polecenia:
...
2. Sprawdź integralność hello systemu plików HDFS w klastrze usługi HDInsight hello za pomocą hello następującego polecenia:
