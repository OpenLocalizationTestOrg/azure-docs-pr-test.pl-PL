...
> [!IMPORTANT]
> Nie trzeba istniejącego klastra usługi HDInsight. Kroki opisane w tym dokumencie utworzenie następujących zasobów:
> 
...
Topologia odczytuje dane z Centrum zdarzeń za pomocą [org.apache.storm.eventhubs.spout.EventHubSpout](http://storm.apache.org/releases/0.10.1/javadocs/org/apache/storm/eventhubs/spout/class-use/EventHubSpout.html) klasy i zapisuje dane do bazy danych HBase przy użyciu [org.apache.storm.hbase.bolt.HBaseBolt](https://storm.apache.org/releases/1.0.1/javadocs/org/apache/storm/hbase/bolt/HBaseBolt.html) klasy. Komunikacja z witryny sieci Web odbywa się przy użyciu [client.java użyciu biblioteki socket.io](https://github.com/nkzawa/socket.io-client.java).
...
Przed skorzystaniem z tego przykładu, należy utworzyć usługi Azure Event Hub, które odczytuje topologii Storm z.

### <a name="configure-event-hub"></a>Konfigurowanie Centrum zdarzeń

Centrum zdarzeń jest źródłem danych, w tym przykładzie. Wykonaj następujące kroki, aby utworzyć Centrum zdarzeń.

1. Z [portalu Azure](https://portal.azure.com), wybierz pozycję **+ nowy** -> **Internetu rzeczy** -> **usługi Event Hubs**.
2. W **utworzyć Namespace** sekcji, wykonaj następujące zadania:
   
   1. Wprowadź **nazwa** dla przestrzeni nazw.
   2. Wybierz warstwę cenową. **Podstawowe** jest wystarczająca dla tego przykładu.
   3. Wybierz platformy Azure **subskrypcji** do użycia.
   4. Wybierz istniejącą grupę zasobów lub Utwórz nową.
   5. Wybierz **lokalizacji** do Centrum zdarzeń.
   6. Wybierz **Przypnij do pulpitu nawigacyjnego**, a następnie kliknij przycisk **Utwórz**.

3. Po zakończeniu procesu tworzenia, zostaną wyświetlone informacje centra zdarzeń, dla przestrzeni nazw. W tym miejscu, wybierz **+ Dodaj Centrum zdarzeń**. W **tworzenia Centrum zdarzeń** sekcji, wprowadź nazwę **sensordata**, a następnie wybierz **Utwórz**. Pozostaw innych pól na wartości domyślne.
4. Z widoku centra zdarzeń w przestrzeni nazw, wybierz **usługi Event Hubs**. Wybierz **sensordata** wpisu.
5. Wybierz z sensordata Centrum zdarzeń **zasady dostępu współużytkowanego**. Użyj **+ Dodaj** łącze, aby dodać następujące zasady:
...
1. Zaznacz obie zasady i zanotuj **klucz podstawowy** wartość. Należy wartość dla obu zasad w przyszłości czynności.
...
> [!WARNING]
> Jeśli używasz środowiska projektowego systemu Windows może zostać wyświetlony `java.io.IOException` podczas uruchamiania topologii lokalnie. Jeśli tak, przejść do uruchamiania topologii w usłudze HDInsight.
...
2. Otwórz `app.js` plik w edytorze tekstów i Dodaj informacje o Centrum zdarzeń uzyskanymi wcześniej:
   ...
   > [!NOTE]
   > W tym przykładzie przyjęto założenie, że użyto `sensordata` jako nazwy Centrum zdarzeń. Oraz że `devices` jako nazwę zasady, która ma `Send` oświadczeń.
...
Aby uzyskać więcej informacji dotyczących bazy danych HBase w usłudze HDInsight, zobacz [HBase HDInsight Przegląd](hdinsight-hbase-overview.md).
