...
W tym artykule opisano model danych usługi Power BI hello używany do tworzenia raportów usługi Kopia zapasowa Azure. Użycie tego modelu danych, można filtrować istniejących raportów na podstawie odpowiednich pól i przy użyciu tabel i pól w modelu hello więcej co ważniejsze, tworzyć własne raporty. 

## <a name="creating-new-reports-in-power-bi"></a>Tworzenie nowych raportów w usłudze Power BI
Usługa Power BI udostępnia funkcje dostosowywania za pomocą którego można [tworzenie raportów przy użyciu modelu danych hello](https://powerbi.microsoft.com/documentation/powerbi-service-create-a-new-report/).

## <a name="using-azure-backup-data-model"></a>Za pomocą usługi Kopia zapasowa Azure modelu danych
Możesz użyć hello kolejnych pól podana jako część danych hello modelu toocreate raporty i dostosowywanie istniejących raportów.
...
| AlertSeverity |Tekst |Ważność alertu hello na przykład krytyczne |
| AlertStatus |Tekst |Stan alertu hello na przykład aktywny |
| AlertType |Tekst |Typ hello wygenerowany alert, na przykład kopii zapasowej |
| AlertUniqueId |Tekst |Unikatowy identyfikator hello wygenerowany alert |
| AsOnDateTime |Data i godzina |Najnowsza wersja Odśwież czas hello wybranego wiersza |
| AvgResolutionTimeInMinsForAlertsCreatedInPeriod |Liczba dziesiętna |Alert tooresolve Średni czas (w minutach) dla wybranego okresu |
| EntityState |Tekst |Bieżący stan obiektu alertu hello na przykład aktywny, usunięte |
...
| EntityState |Tekst |Bieżący stan obiektu kopii zapasowej elementu hello na przykład aktywny, usunięte |
...
| ProtectionState |Tekst |Bieżący stan ochrony hello elementu kopii zapasowej, na przykład chronione, ProtectionStopped |
...
| Miesiąc |Tekst |Miesiąc roku hello wybrane do filtrowania danych, rozpoczyna się pierwszego dnia miesiąca i kończy się dzień 31- |
| MonthDate | Date |Data w miesiącu hello zakończenia miesiąca wybrane do filtrowania danych |
...
| WeekDate |Date |Data w tygodniu hello, gdy kończy się tydzień wybrane do filtrowania danych |
...
| YearDate |Date |Data w roku hello, gdy kończy się roku wybrane do filtrowania danych |
...
| Pole | Typ danych | Opis |
| --- | --- | --- |
| #JobsCreatedInPeriod |Liczby całkowitej |Liczba zadań utworzonych w wybranym okresie hello |
| % FailuresForJobsCreatedInPeriod |Wartość procentowa |Wartość procentowa ogólne zadania błędów w wybranym okresie hello |
| 80thPercentileDataTransferredInMBForBackupJobsCreatedInPeriod |Liczba dziesiętna |80. wartość percentylu danych przesyłanych w MB **kopii zapasowej** zadania utworzone w wybranym okresie hello |
| AsOnDateTime |Data i godzina |Najnowsza wersja Odśwież czas hello wybranego wiersza |
...
| EntityState |Tekst |Bieżący stan obiektu zadania hello na przykład aktywny, usunięte |
...
| JobStatus |Tekst |Stan hello zakończył na przykład zadanie, została zakończona, nie powiodło się |
| JobUniqueId |Tekst |Unikatowy identyfikator tooidentify hello zadania |
...
| Pole | Typ danych | Opis |
| --- | --- | --- |
| #Policies |Liczby całkowitej |Liczba zasad tworzenia kopii zapasowych, które istnieją w systemie hello |
| #PoliciesInUse |Liczby całkowitej |Liczba zasad aktualnie używane do konfigurowania kopii zapasowych |
| AsOnDateTime |Data i godzina |Najnowsza wersja Odśwież czas hello wybranego wiersza |
| BackupDaysOfTheWeek |Tekst |Dni tygodnia hello podczas zaplanowanych kopii zapasowych |
...
| EntityState |Tekst |Bieżący stan obiektu zasad hello na przykład aktywny, usunięte |
| MonthlyRetentionDaysOfTheMonth |Tekst |Daty miesiąca hello wybrany do przechowywania miesięcznego |
| MonthlyRetentionDaysOfTheWeek |Tekst |Wybrane dni tygodnia hello do przechowywania miesięcznego |
...
| MonthlyRetentionWeeksOfTheMonth |Tekst |Tygodnie miesiąca hello podczas przechowywania miesięcznych jest na przykład skonfigurować First, Last itp. |
| PolicyName |Tekst |Nazwa zasad hello zdefiniowany |
| PolicyUniqueId |Tekst |Unikatowy identyfikator tooidentify hello zasad |
| RetentionType |Tekst |Typ zasad przechowywania na przykład codziennie, co tydzień, co miesiąc, co rok |
| WeeklyRetentionDaysOfTheWeek |Tekst |Wybrane dni tygodnia hello do przechowywania co tydzień |
...
| YearlyRetentionDaysOfTheMonth |Tekst |Daty miesiąca hello wybrany do przechowywania roczne |
| YearlyRetentionDaysOfTheWeek |Tekst |Wybrane dni tygodnia hello do przechowywania roczne |
...
| YearlyRetentionMonthsOfTheYear |Tekst |Miesięcy roku hello wybrany do przechowywania roczne |
| YearlyRetentionTimes |Tekst |Data i godzina, gdy corocznych przechowywania jest skonfigurowany |
| YearlyRetentionWeeksOfTheMonth |Tekst |Tygodnie miesiąca hello podczas przechowywania corocznych jest na przykład skonfigurować First, Last itp. |
...
| EntityState |Tekst |Bieżący stan obiektu serwera chronionego hello na przykład aktywny, usunięte |
...
| ProtectedServerName |Tekst |Nazwa elementu kopii zapasowych serwera chronionego toowhich należy |
...
| EntityState |Tekst |Bieżący stan obiektu hello na przykład aktywny, usunięte |
...
| Pole | Typ danych | Opis |
| --- | --- | --- |
| Godzina |Time |Godzina dnia hello na przykład 1:00:00 PM |
| HourNumber |Liczba dziesiętna |Numer godzinę w ciągu dnia hello na przykład 13,00 |
| Minuta |Liczba dziesiętna |Minuta godziny hello |
| PeriodOfTheDay |Tekst |Odstęp okresu czasu w ciągu dnia hello na przykład AM 12-3 |
| Time |Time |Pora dnia hello na przykład 00:00:01: 00 |
| TimeKey |Tekst |Wartość klucza toorepresent czasu |
...
| EntityState |Tekst |Bieżący stan obiektu magazynu hello na przykład aktywny, usunięte |
| StorageReplicationType |Tekst |Typ replikacji magazynu dla magazynu hello na przykład GeoRedundant |
| SubscriptionId |Tekst |Identyfikator subskrypcji klienta hello wybranego do generowania raportów |
| VaultName |Tekst |Nazwa magazynu hello |
| VaultTags |Tekst |Znaczniki skojarzone toohello magazynu |

## <a name="next-steps"></a>Następne kroki
Po przejrzeniu hello modelu danych do tworzenia raportów usługi Kopia zapasowa Azure można znaleźć hello następujące artykuły, aby uzyskać więcej informacji na temat tworzenia i wyświetlania raportów w usłudze Power BI.
