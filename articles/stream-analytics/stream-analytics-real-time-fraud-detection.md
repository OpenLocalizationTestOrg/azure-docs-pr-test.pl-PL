...
W tej procedurze należy najpierw utworzyć przestrzeń nazw Centrum zdarzeń, a następnie dodaj Centrum zdarzeń do tego namepsace. Przestrzenie nazw Centrum zdarzeń są używane do logicznego grupowania wystąpień magistrali powiązanych zdarzeń. 

1. Zaloguj się do portalu Azure i kliknij przycisk **nowy** > **Internetu rzeczy** > **Centrum zdarzeń**. 

2. W **tworzenie przestrzeni nazw** bloku, wprowadź nazwę przestrzeni nazw, takich jak `<yourname>-eh-ns-demo`. Można użyć dowolnej nazwy przestrzeni nazw, ale nazwa musi być prawidłowa dla danego adresu URL i między Azure musi być unikatowa. 
...
1. Na pulpicie nawigacyjnym lub **wszystkie zasoby** bloku, Znajdź i wybierz `sa_frauddetection_job_demo` zadania usługi analiza strumienia. 
2. W **topologii zadania** sekcji bloku zadania Stream Analytics, kliknij przycisk **dane wejściowe** pole.
...
3. Kliknij przycisk  **+ &nbsp;Dodaj** , a następnie wypełnij bloku z tymi wartościami:
...
Bardzo proste zapytanie po prostu może odczytać wszystkich danych przychodzących. Jednak często tworzyć zapytania, które wyglądają dla określonych danych lub w przypadku relacji w danych. W tej części samouczka utworzysz i przetestować kilka zapytań, aby dowiedzieć się więcej na kilka sposobów, w których można przekształcać strumień wejściowy do analizy. 
...
2. W portalu wróć do bloku zadanie analizy przesyłania strumieniowego. (Jeśli został zamknięty bloku, wyszukaj `sa_frauddetection_job_demo` w **wszystkie zasoby** bloku.)
...
4. W **zapytania** bloku, kliknij punkty `CallStream` danych wejściowych, a następnie wybierz **przykładowe dane z danych wejściowych**.
...
1. W portalu Azure wróć do bloku zadanie analizy przesyłania strumieniowego. (Jeśli został zamknięty bloku, wyszukaj `sa_frauddetection_job_demo` w **wszystkie zasoby** bloku.)
2. W **topologii zadania** kliknij **dane wyjściowe** pole. 
3. W **dane wyjściowe** bloku, kliknij przycisk  **+ &nbsp;Dodaj** , a następnie wypełnij bloku z tymi wartościami:
...
3. W **rozpoczęcia zadania** bloku dla danych wyjściowych czas uruchomienia zadania, wybierz opcję **teraz**. 
...
1. Zatrzymaj zadanie analizy przesyłania strumieniowego. W **zadania** bloku, kliknij przycisk **zatrzymać** u góry.
...
* [Jak przechowywać dane z usługi Azure Stream Analytics w pamięci podręcznej Redis Azure za pomocą usługi Azure Functions](stream-analytics-functions-redis.md). W tym artykule pokazano, jak użyć usługi Azure Functions, aby zapisać fałszywych wywołań Azure pamięcią podręczną Redis za pośrednictwem kolejki usługi Service Bus.
