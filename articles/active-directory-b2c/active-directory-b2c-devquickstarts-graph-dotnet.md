...
---
# <a name="azure-ad-b2c-use-the-graph-api"></a>Usługi Azure AD B2C: Interfejs API Graph użycia
...
6. W bloku ustawienia kliknij **klucze** i Dodaj nowy klucz (znanej także jako klucz tajny klienta). Jest on również kopiowany do użycia w dalszej części artykułu.
...
1. Kontynuowanie w bloku rejestracji aplikacji w portalu Azure, wybierz aplikację.
2. W bloku ustawienia kliknij **wymagane uprawnienia**.
3. W bloku wymaganych uprawnień, kliknij na **Windows Azure Active Directory**.
4. W bloku Włącz dostęp wybierz **Odczyt i zapis danych katalogu** zgody **uprawnienia aplikacji** i kliknij przycisk **zapisać**.
5. Ponadto w bloku wymaganych uprawnień, kliknij na **udzielanie uprawnień** przycisku.
...
Najpierw należy pobrać i zainstalować [Microsoft Online Services Asystenta logowania](http://go.microsoft.com/fwlink/?LinkID=286152). Następnie Pobierz i zainstaluj [64-bitowy moduł usługi Azure Active Directory dla środowiska Windows PowerShell](http://go.microsoft.com/fwlink/p/?linkid=236297).

Po zainstalowaniu modułu programu PowerShell, Otwórz program PowerShell i połącz się dzierżawy usługi B2C. Po uruchomieniu `Get-Credential`, zostanie wyświetlony monit dla nazwy użytkownika i hasła, wprowadź nazwę użytkownika i hasło konta administratora dzierżawy B2C.
