...
Witaj sekcje, które wykonuje przedstawimy standardowymi elementami, które zwykle znajdują się w tych strefach.
...
Firma Microsoft używa framework hello opisanych powyżej zagrożeń toodo modelowania dla Azure IoT. W poniższej sekcji hello w związku z tym używany hello konkretny przykład sposób identyfikowania toothink o modelowania zagrożeń IoT i jak tooaddress hello zagrożenia toodemonstrate architektura referencyjna IoT platformy Azure. W tym przypadku określiliśmy cztery główne obszary fokus:
...
System kontroli (lub kontrolera) jest rozwiązaniem oprogramowania, które interfejsy z urządzenia, lub brama pola lub brama chmury hello w celu kontrolowania jednego lub wielu urządzeń i/lub toocollect i/lub magazynu i/lub analizowania danych urządzeń do prezentacji, lub celów kolejnych kontroli. Systemów kontroli są hello jednostek tylko w zakresie hello tej dyskusji, która może ułatwić natychmiast interakcji z użytkownikami. wyjątek Hello są pośredniego fizycznych powierzchni na urządzeniach, takich jak przełącznik, który umożliwia osoby tooturn hello urządzenia poza lub zmienić inne właściwości i dla którego nie ma odpowiednika funkcjonalności, które mogą uzyskiwać cyfrowo. 
