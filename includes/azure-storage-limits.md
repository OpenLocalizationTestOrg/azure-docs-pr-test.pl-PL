...
| Maksymalna liczba plików w udziale plików |Tylko limit wynosi 5 TB hello całkowita pojemność hello udziału plików |
...
| Maksymalna szybkość żądania na konto magazynu |Obiekty BLOB: 20 000 żądań na sekundę<sup>2</sup> dla obiektów blob o dowolnym rozmiarze prawidłowe (ograniczone tylko przez limity wejście/wyjście hello konta) <br />Pliki: 1000 IOPS (o rozmiarze 8 KB) dla udziału plików <br />Kolejki: 20 000 komunikatów na sekundę (przyjmuje rozmiar komunikatu o rozmiarze 1 KB)<br />Tabele: 20 000 transakcji na sekundę (przyjmuje rozmiar jednostki o rozmiarze 1 KB) |
| Przepływność docelowy dla pojedynczego obiektu blob |Zapasowej too60 MB na sekundę lub w górę too500 żądań na sekundę |
| Przepływność docelowy dla pojedynczej kolejki (1 KB wiadomości) |Zapasowej too2000 komunikatów na sekundę |
| Przepływność docelowy dla pojedynczej tabeli partycji (o rozmiarze 1 KB jednostki) |Zapasowej too2000 jednostek na sekundę |
| Docelowy przepływności jeden udział pliku |Too60 MB na sekundę |
...
<sup>2</sup> tooget toogrow ostatnich kont magazynu w warstwie standardowa hello anonsowany limity szybkości pojemności, wejście/wyjście i żądania, należy wysłać żądanie za pośrednictwem [pomocą techniczną platformy Azure](https://azure.microsoft.com/support/faq/). Hello Azure Storage team przejrzeć żądania hello i może zatwierdzić wyższe limity na podstawie przypadku.

<sup>3</sup>*wejściowych* odwołuje się tooall danych (liczba żądań) są wysyłane tooa konta magazynu. *Transfer danych wychodzących* odwołuje się tooall danych (odpowiedzi) odbierane z konta magazynu.  

<sup>4</sup>dostępne są następujące opcje replikacji usługi azure Storage:
* **RA-GRS**: dostęp do odczytu z magazynu geograficznie nadmiarowego magazynu. Jeśli włączono RA-GRS, docelowe wyjście lokalizacji dodatkowej hello są identyczne toothose do lokalizacji głównej hello.
